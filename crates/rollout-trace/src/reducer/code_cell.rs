...
//! - runtime work starts as soon as Sprite dispatches the tool.
//!
...
pub(super) struct PendingCodeCellStart {
    ...
    pub(super) runtime_activation_id: Option<String>,
...
impl TraceReducer {
    ...
    fn start_code_cell(&mut self, pending: PendingCodeCellStart) -> Result<()> {
        ...
        let Some(runtime_activation_id) = runtime_activation_id else {
            bail!(
                "code cell start {} did not include a runtime activation id",
                started.code_cell_id
            );
        };
        self.validate_code_cell_turn(&thread_id, &runtime_activation_id)?;
    ...
    pub(super) fn terminate_running_code_cells_for_turn_end(
        ...
        let code_cell_ids: Vec<_> = self
            ...
            .values()
            .filter(|cell| {
                cell.runtime_activation_id == runtime_activation_id
    ...
    pub(super) fn code_cell_event_thread_id(
        ...
        runtime_activation_id: Option<&str>,
        runtime_cell_id: &str,
        event_name: &str,
    ) -> Result<String> {
        ...
        self.rollout
            .runtime_activations
            .get(runtime_activation_id)
            .map(|turn| turn.thread_id.clone())
            .with_context(|| {
                format!(
                    "{event_name} {runtime_cell_id} referenced unknown runtime activation {runtime_activation_id}"
                )
            })
    }
    ...
    fn validate_code_cell_turn(&self, thread_id: &str, runtime_activation_id: &str) -> Result<()> {
        if !self.rollout.threads.contains_key(thread_id) {
            bail!("code cell start referenced unknown thread {thread_id}");
        }
        let Some(turn) = self.rollout.runtime_activations.get(runtime_activation_id) else {
            bail!("code cell start referenced unknown runtime activation {runtime_activation_id}");
        };
        if turn.thread_id != thread_id {
            bail!(
                "code cell start used thread {thread_id}, but runtime activation {runtime_activation_id} belongs \
                 to {}",
                turn.thread_id
            );
        }
