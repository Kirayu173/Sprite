...
impl TraceReducer {
    ...
    pub(super) fn start_compaction_request(
        ...
        let Some(turn) = self
            .rollout
            .runtime_activations
            .get(&started.runtime_activation_id)
        else {
            bail!(
                "compaction request {} referenced unknown runtime activation {}",
                started.compaction_request_id,
                started.runtime_activation_id
            );
        };
        if turn.thread_id != started.thread_id {
            bail!(
                "compaction request {} used thread {}, but runtime activation {} belongs to {}",
                started.compaction_request_id,
                started.thread_id,
                started.runtime_activation_id,
                turn.thread_id
        ...
        self.rollout.compaction_requests.insert(
            ...
            CompactionRequest {
                ...
                runtime_activation_id: started.runtime_activation_id,
    ...
    pub(super) fn reduce_compaction_installed_event(
        ...
        let Some(turn) = self.rollout.runtime_activations.get(&runtime_activation_id) else {
            bail!(
                "compaction install {compaction_id} referenced unknown runtime activation {runtime_activation_id}"
            );
        };
        if turn.thread_id != thread_id {
            bail!(
                "compaction install {compaction_id} used thread {thread_id}, but runtime activation {runtime_activation_id} belongs to {}",
                turn.thread_id
            );
        }
...
pub(super) struct StartedCompactionRequest {
    ...
    pub(super) runtime_activation_id: String,
