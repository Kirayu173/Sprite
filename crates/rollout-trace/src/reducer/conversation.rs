...
impl TraceReducer {
    ...
    pub(super) fn reduce_inference_request(
        ...
        // history, not the pre-compaction prompt. Any repeated developer/context prefix that Sprite
    ...
    pub(super) fn reduce_inference_response(
        ...
        let Some((thread_id, runtime_activation_id)) = self
            .rollout
            .inference_calls
            ...
            .map(|inference| {
                (
                    ...
                    inference.runtime_activation_id.clone(),
                )
            })
        else {
        ...
        for (offset, item) in items.into_iter().enumerate() {
            ...
            let item_id = if let Some(previous_item_id) = previous_snapshot.get(index) {
                if self.item_matches(previous_item_id, &item) {
                    ...
                    self.find_matching_snapshot_item(&previous_snapshot, &item_ids, &item)
                        .unwrap_or_else(|| {
                            self.create_conversation_item(
                                context.thread_id,
                                Some(context.runtime_activation_id.to_string()),
                    ...
                    let runtime_activation_id = context.runtime_activation_id;
                    let thread_id = context.thread_id;
                    bail!(
                        "model conversation mismatch while reducing turn {runtime_activation_id} for \
    ...
    pub(super) fn reduce_compaction_checkpoint(
        ...
        let marker_item_id = self.create_conversation_item(
            thread_id,
            Some(runtime_activation_id.to_string()),
            wall_time_unix_ms,
