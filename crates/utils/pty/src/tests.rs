...
async fn spawn_pty_process_or_skip_if_unavailable(
    test_name: &str,
    program: &str,
    args: &[String],
    cwd: &Path,
    ...
    arg0: &Option<String>,
    size: TerminalSize,
) -> anyhow::Result<Option<SpawnedProcess>> {
    match spawn_pty_process(program, args, cwd, env, arg0, size).await {
        Ok(spawned) => Ok(Some(spawned)),
        Err(error) if is_platform_pty_unavailable(&error) => {
            eprintln!("ConPTY unavailable; skipping {test_name}: {error:#}");
            Ok(None)
        }
        Err(error) => Err(error),
    }
...
fn is_platform_pty_unavailable(error: &anyhow::Error) -> bool {
    cfg!(windows)
        && error.chain().any(|cause| {
            let message = cause.to_string();
            message.contains("failed to create psuedo console")
                || message.contains("failed to create pseudo console")
        })
}
...
async fn wait_for_python_repl_ready_via_probe(
    ...
    let mut collected = Vec::new();
    let marker = "__sprite_pty_ready__";
    let deadline = tokio::time::Instant::now() + tokio::time::Duration::from_millis(timeout_ms);
...
async fn pty_python_repl_emits_output_and_exits() -> anyhow::Result<()> {
    ...
    let ready_marker = "__sprite_pty_ready__";
    let args = vec![
        "-i".to_string(),
        "-q".to_string(),
        "-c".to_string(),
    ...
    let Some(spawned) = spawn_pty_process_or_skip_if_unavailable(
...
async fn pipe_and_pty_share_interface() -> anyhow::Result<()> {
    ...
    let Some(pty) = spawn_pty_process_or_skip_if_unavailable(
...
async fn pty_preserving_inherited_fds_keeps_python_repl_running() -> anyhow::Result<()> {
    ...
    .await?;
    let marker = "__sprite_preserved_py_pid:";
    writer
