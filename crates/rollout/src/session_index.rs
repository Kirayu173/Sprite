...
pub async fn append_thread_name(
    ...
    append_session_index_entry(sprite_home, &entry).await
}
...
pub async fn append_session_index_entry(
    ...
    let path = session_index_path(sprite_home);
...
fn session_index_path(sprite_home: &Path) -> PathBuf {
    sprite_home.join(SESSION_INDEX_FILE)
}
