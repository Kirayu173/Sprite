...
async fn wait_for_backfill_gate(
    ...
    loop {
        ...
        if let Some(backfill_lease_seconds) = backfill_lease_seconds {
            ...
            metadata::backfill_sessions(runtime, sprite_home, default_model_provider_id).await;
        }
        let backfill_state = runtime.get_backfill_state().await.map_err(|err| {
            anyhow::anyhow!(
...
/// Build a SQLite diagnostics recorder backed by an OTEL metrics client.
pub fn sqlite_diagnostics_recorder(
    metrics: diagnostics::MetricsClient,
    originator: &str,
) -> state::DbDiagnosticsHandle {
...
fn cursor_to_anchor(cursor: Option<&Cursor>) -> Option<state::Anchor> {
    let cursor = cursor?;
...
pub async fn list_thread_ids_db(
    context: Option<&state::StateRuntime>,
    sprite_home: &Path,
    page_size: usize,
    ...
    let ctx = context?;
    if ctx.sprite_home() != sprite_home {
        warn!(
            "state db sprite_home mismatch: expected {}, got {}",
            ctx.sprite_home().display(),
            sprite_home.display()
        );
    }
    ...
    match ctx
        .list_thread_ids(
            ...
            anchor.as_ref(),
            match sort_key {
                ThreadSortKey::CreatedAt => state::SortKey::CreatedAt,
                ThreadSortKey::UpdatedAt => state::SortKey::UpdatedAt,
            },
        ...
        .list_threads(
            ...
            state::ThreadFilterOptions {
                ...
                sort_direction: match sort_direction {
                    SortDirection::Asc => state::SortDirection::Asc,
                    SortDirection::Desc => state::SortDirection::Desc,
                },
                search_term,
            },
        )
