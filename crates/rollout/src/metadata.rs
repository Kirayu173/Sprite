...
pub(crate) async fn backfill_sessions_with_lease(
    ...
    let metric_client = diagnostics::global();
    ...
    let sessions_root = sprite_home.join(SESSIONS_SUBDIR);
        ...
        match collect_rollout_paths(&root).await {
            Ok(paths) => {
                rollout_paths.extend(paths.into_iter().map(|path| BackfillRolloutPath {
                    watermark: backfill_watermark_for_path(sprite_home, &path),
                    path,
                    archived,
                }));
            }
...
fn backfill_watermark_for_path(sprite_home: &Path, path: &Path) -> String {
    path.strip_prefix(sprite_home)
        .unwrap_or(path)
        .to_string_lossy()
        .replace('\\', "/")
}
