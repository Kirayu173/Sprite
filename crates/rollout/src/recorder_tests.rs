...
fn test_config(sprite_home: &Path) -> RolloutConfig {
    RolloutConfig {
        sprite_home: sprite_home.to_path_buf(),
        sqlite_home: sprite_home.to_path_buf(),
        cwd: sprite_home.to_path_buf(),
...
async fn load_rollout_items_preserves_legacy_auto_review_assessment_lines() -> std::io::Result<()> {
    ...
    assert_eq!(parse_errors, 0);
    assert_eq!(items.len(), 2);
    let RolloutItem::EventMsg(EventMsg::AutoReviewAssessment(assessment)) = &items[1] else {
        panic!("expected auto review assessment rollout item");
    };
    assert_eq!(assessment.id, "auto-review-1");
