//! Persist Sprite session rollouts (.jsonl) so sessions can be replayed or inspected later.
...
use git_utils::get_git_repo_root;
use runtime_protocol::protocol::GitInfo as ProtocolGitInfo;
use runtime_protocol::protocol::InitialHistory;
use runtime_protocol::protocol::MultiAgentVersion;
use runtime_protocol::protocol::ResumedHistory;
use runtime_protocol::protocol::RolloutItem;
use runtime_protocol::protocol::RolloutLine;
...
use runtime_protocol::protocol::ThreadSource;
use state::StateRuntime;
use utils_path as path_utils;
...
///
/// ```ignore
/// $ jq -C . ~/.sprite/sessions/rollout-2025-05-07T17-24-21-5973b6c0-94b8-487b-a530-2aeb6098ae0e.jsonl
/// $ fx ~/.sprite/sessions/rollout-2025-05-07T17-24-21-5973b6c0-94b8-487b-a530-2aeb6098ae0e.jsonl
/// ```
#[derive(Clone)]
...
impl RolloutRecorder {
    /// List threads (rollout files) under the provided Sprite home directory.
    ...
    async fn list_threads_with_db_fallback(
        ...
        let sprite_home = config.sprite_home();
...
async fn list_threads_from_files_desc(
    ...
    if let Some(search_term) = search_term {
        ...
        loop {
            ...
            filter_thread_items_by_search_term(sprite_home, &mut page.items, Some(search_term))
                .await?;
...
async fn list_threads_from_files_desc_unfiltered(
    ...
    if archived {
        let root = sprite_home.join(ARCHIVED_SESSIONS_SUBDIR);
        get_threads_in_root(
            root,
            page_size,
            cursor,
...
async fn list_threads_from_files_asc(
    ...
    filter_thread_items_by_search_term(sprite_home, &mut all_items, search_term).await?;
...
async fn filter_thread_items_by_search_term(
    ...
    let thread_ids = items
        ...
        .filter_map(|item| item.thread_id)
        .collect::<HashSet<_>>();
    let thread_names = find_thread_names_by_ids(sprite_home, &thread_ids).await?;
    items.retain(|item| {
        item.thread_id
...
fn precompute_log_file_info(
    ...
    // Resolve ~/.sprite/sessions/YYYY/MM/DD path.
    let timestamp = OffsetDateTime::now_local()
        .map_err(|e| IoError::other(format!("failed to get local time: {e}")))?;
    let mut dir = config.sprite_home().to_path_buf();
...
impl From<state::ThreadsPage> for ThreadsPage {
    fn from(db_page: state::ThreadsPage) -> Self {
        let items = db_page
            .items
            .into_iter()
...
fn thread_item_from_state_metadata(item: state::ThreadMetadata) -> ThreadItem {
    ThreadItem {
