...
pub async fn get_threads(
    ...
    let root = sprite_home.join(SESSIONS_SUBDIR);
    get_threads_in_root(
        root,
        page_size,
        cursor,
...
///
/// Directory layout: `~/.sprite/sessions/YYYY/MM/DD/rollout-YYYY-MM-DDThh-mm-ss-<uuid>.jsonl`
...
async fn find_thread_path_by_id_str_in_subdir(
    ...
    subdir: &str,
    id_str: &str,
    state_db_ctx: Option<&state::StateRuntime>,
    ...
    let mut root = sprite_home.to_path_buf();
    root.push(subdir);
    if !root.exists() {
...
pub async fn find_thread_path_by_id_str(
    ...
    find_thread_path_by_id_str_in_subdir(sprite_home, SESSIONS_SUBDIR, id_str, state_db_ctx).await
}
