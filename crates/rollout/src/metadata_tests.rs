...
async fn backfill_sessions_resumes_from_watermark_and_marks_complete() {
    let dir = tempdir().expect("tempdir");
    let sprite_home = dir.path().to_path_buf();
    ...
    let runtime = state::StateRuntime::init(sprite_home.clone(), "test-provider".to_string())
        .await
        .expect("initialize runtime");
    let first_watermark = backfill_watermark_for_path(sprite_home.as_path(), first_path.as_path());
    ...
    backfill_sessions(runtime.as_ref(), sprite_home.as_path(), "test-provider").await;
...
async fn backfill_sessions_normalizes_cwd_before_upsert() {
    ...
    let sprite_home = dir.path().to_path_buf();
    let thread_uuid = Uuid::new_v4();
    let session_cwd = sprite_home.join(".");
...
fn write_rollout_in_sessions_with_cwd(
    ...
    let sessions_dir = sprite_home.join("sessions");
