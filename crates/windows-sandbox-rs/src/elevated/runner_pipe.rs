...
pub fn find_runner_exe(sprite_home: &Path, log_dir: Option<&Path>) -> PathBuf {
    resolve_helper_for_launch(HelperExecutable::CommandRunner, sprite_home, log_dir)
}
...
pub fn pipe_pair() -> (String, String) {
    ...
    let base = format!(r"\\.\pipe\sprite-runner-{nonce:x}");
