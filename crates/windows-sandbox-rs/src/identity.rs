...
pub fn sandbox_setup_is_complete(sprite_home: &Path) -> bool {
    let marker_ok =
        matches!(load_marker(sprite_home), Ok(Some(marker)) if marker.version_matches());
    if !marker_ok {
        return false;
    }
    matches!(load_users(sprite_home), Ok(Some(users)) if users.version_matches())
}

fn load_marker(sprite_home: &Path) -> Result<Option<SetupMarker>> {
    let path = setup_marker_path(sprite_home);
...
fn load_users(sprite_home: &Path) -> Result<Option<SandboxUsersFile>> {
    let path = sandbox_users_path(sprite_home);
...
fn select_identity(
    ...
    let _marker = match load_marker(sprite_home)? {
        Some(m) if m.version_matches() => m,
        _ => return Ok(None),
    };
    let users = match load_users(sprite_home)? {
...
pub fn require_logon_sandbox_creds(
    ...
    let sandbox_dir = crate::setup::sandbox_dir(sprite_home);
    let needed_read = read_roots_override
        .map(<[PathBuf]>::to_vec)
        .unwrap_or_else(|| gather_read_roots(command_cwd, permissions, env_map, sprite_home));
    ...
    // NOTE: Do not add SPRITE_HOME/.sandbox to `needed_write`; it must remain non-writable by the
    ...
    let mut identity = match load_marker(sprite_home)? {
        Some(marker) if marker.version_matches() => {
                ...
                marker.request_mismatch_reason(network_identity, &desired_offline_proxy_settings)
            {
                ...
                let selected = select_identity(network_identity, sprite_home)?;
                if selected.is_none() {
                    setup_reason = Some(
    ...
    if identity.is_none() {
        ...
        identity = select_identity(network_identity, sprite_home)?;
    }
