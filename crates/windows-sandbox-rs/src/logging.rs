...
use utils_string::take_bytes_at_char_boundary;
...
pub fn current_log_file_path_for_sprite_home(sprite_home: &Path) -> PathBuf {
    current_log_file_path(&crate::sandbox_dir(sprite_home))
}
...
mod tests {
    ...
    #[test]
    fn current_log_file_path_for_sprite_home_uses_sandbox_dir() {
        let sprite_home = Path::new("sprite-home");

        assert_eq!(
            current_log_file_path_for_sprite_home(sprite_home),
            current_log_file_path(&sprite_home.join(".sandbox"))
        );
    }
}
