...
pub(super) fn ensure_sprite_app_runtime_bin_readable(
    ...
    // Sprite desktop copies bundled Windows binaries out of WindowsApps to this
    // fixed LocalAppData cache before launching sprite.exe.
    let runtime_bin_dir = local_app_data.join("Sprite").join("bin");
