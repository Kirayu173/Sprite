...
use sprite_windows_sandbox::dpapi_protect;
...
pub const SANDBOX_USERS_GROUP: &str = "SpriteSandboxUsers";
const SANDBOX_USERS_GROUP_COMMENT: &str = "Sprite sandbox internal group (managed)";
...
fn write_secrets(
    ...
    let secrets_dir = sandbox_secrets_dir(sprite_home);
...
pub(super) fn prepare_setup_marker(sprite_home: &Path, real_user: &str) -> Result<()> {
    let marker_path = sandbox_dir(sprite_home).join("setup_marker.json");
