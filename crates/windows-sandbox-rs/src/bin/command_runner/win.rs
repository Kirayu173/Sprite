...
use anyhow::Context;
use anyhow::Result;
use sprite_windows_sandbox::ErrorPayload;
use sprite_windows_sandbox::ExitPayload;
use sprite_windows_sandbox::FramedMessage;
use sprite_windows_sandbox::IPC_PROTOCOL_VERSION;
use sprite_windows_sandbox::LocalSid;
use sprite_windows_sandbox::Message;
use sprite_windows_sandbox::OutputPayload;
use sprite_windows_sandbox::OutputStream;
use sprite_windows_sandbox::PipeSpawnHandles;
use sprite_windows_sandbox::ResizePayload;
use sprite_windows_sandbox::SpawnReady;
use sprite_windows_sandbox::SpawnRequest;
use sprite_windows_sandbox::StderrMode;
use sprite_windows_sandbox::StdinMode;
use sprite_windows_sandbox::WindowsSandboxTokenMode;
use sprite_windows_sandbox::allow_null_device;
use sprite_windows_sandbox::create_readonly_token_with_caps_and_user_from;
use sprite_windows_sandbox::create_workspace_write_token_with_caps_and_user_from;
use sprite_windows_sandbox::decode_bytes;
use sprite_windows_sandbox::encode_bytes;
use sprite_windows_sandbox::get_current_token_for_restriction;
use sprite_windows_sandbox::hide_current_user_profile_dir;
use sprite_windows_sandbox::log_note;
use sprite_windows_sandbox::read_frame;
use sprite_windows_sandbox::read_handle_loop;
use sprite_windows_sandbox::spawn_process_with_pipes;
use sprite_windows_sandbox::to_wide;
use sprite_windows_sandbox::token_mode_for_permission_profile;
use sprite_windows_sandbox::write_frame;
use std::ffi::OsStr;
use std::fs::File;
...
const READ_ACL_MUTEX_NAME: &str = "Local\\SpriteSandboxReadAcl";
...
struct IpcSpawnedProcess {
    ...
    conpty_owner: Option<sprite_windows_sandbox::ConptyInstance>,
...
fn spawn_ipc_process(req: &SpawnRequest) -> Result<IpcSpawnedProcess> {
    let log_dir = req.sprite_home.clone();
    hide_current_user_profile_dir(req.sprite_home.as_path());
    ...
    let (pi, stdout_handle, stderr_handle, stdin_handle) = if req.tty {
        let (pi, mut conpty) = sprite_windows_sandbox::spawn_conpty_process_as_user(
            h_token.raw(),
            &req.command,
            &effective_cwd,
            &req.env,
