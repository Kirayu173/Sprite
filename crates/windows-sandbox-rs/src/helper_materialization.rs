...
pub(crate) const RESOURCES_DIRNAME: &str = "sprite-resources";
...
impl HelperExecutable {
    fn file_name(self) -> &'static str {
        match self {
            Self::CommandRunner => "sprite-command-runner.exe",
        }
    }
...
pub(crate) fn helper_bin_dir(sprite_home: &Path) -> PathBuf {
    sandbox_bin_dir(sprite_home)
}
...
pub(crate) fn resolve_helper_for_launch(
    ...
    match copy_helper_if_needed(kind, sprite_home, log_dir) {
        Ok(path) => {
            log_note(
                &format!(
...
pub fn resolve_current_exe_for_launch(sprite_home: &Path, fallback_executable: &str) -> PathBuf {
    ...
    let destination = helper_bin_dir(sprite_home).join(file_name);
    match copy_from_source_if_needed(&source, &destination) {
        Ok(_) => destination,
        Err(err) => {
            let sandbox_log_dir = crate::sandbox_dir(sprite_home);
            log_note(
                &format!(
...
pub(crate) fn copy_helper_if_needed(
    ...
    let cache_key = format!("{}|{}", kind.file_name(), sprite_home.display());
    ...
    let destination = helper_destination_for_source(kind, sprite_home, &source)?;
    log_note(
        &format!(
...
fn helper_destination_for_source(
    ...
    Ok(helper_bin_dir(sprite_home).join(file_name))
}
...
mod tests {
    ...
    fn helper_bin_dir_is_under_sandbox_bin() {
        let sprite_home = Path::new(r"C:\Users\example\.sprite");

        assert_eq!(
            PathBuf::from(r"C:\Users\example\.sprite\.sandbox-bin"),
            helper_bin_dir(sprite_home)
        );
    }
    ...
    fn copy_runner_into_shared_bin_dir() {
        ...
        let runner_source = source_dir.join("sprite-command-runner.exe");
        fs::write(&runner_source, b"runner").expect("runner");
        let runner_suffix = helper_version_suffix(&runner_source).expect("runner suffix");
        let runner_destination = helper_bin_dir(&sprite_home).join(materialized_file_name(
    ...
    fn helper_source_lookup_checks_resource_dir() {
        ...
        let exe = release_dir.join("sprite.exe");
        let helper = resources_dir.join("sprite-command-runner.exe");
        fs::write(&exe, b"codex").expect("write exe");
        fs::write(&helper, b"runner").expect("write helper");

        let resolved =
            bundled_executable_path_for_exe(&exe, /*file_name*/ "sprite-command-runner.exe")
                .expect("helper path");
    ...
    fn helper_source_lookup_prefers_package_resource_dir_over_bin_resource_dir() {
        ...
        let exe = bin_dir.join("sprite.exe");
        let package_helper = package_resources_dir.join("sprite-command-runner.exe");
        let bin_helper = bin_resources_dir.join("sprite-command-runner.exe");
        fs::write(&exe, b"codex").expect("write exe");
    ...
    fn helper_source_lookup_prefers_direct_sibling_over_resource_dir() {
        ...
        fs::create_dir_all(&resources_dir).expect("create resources dir");
        let exe = release_dir.join("sprite.exe");
        let sibling_helper = release_dir.join("sprite-command-runner.exe");
        let resource_helper = resources_dir.join("sprite-command-runner.exe");
        fs::write(&exe, b"codex").expect("write exe");
    ...
    fn materialized_file_name_adds_suffix_before_extension() {
        ...
        assert_eq!(file_name, "sprite-command-runner-test-suffix.exe");
    }
}
