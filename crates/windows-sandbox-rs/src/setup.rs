...
pub const OFFLINE_USERNAME: &str = "SpriteSandboxOffline";
pub const ONLINE_USERNAME: &str = "SpriteSandboxOnline";
const ERROR_CANCELLED: u32 = 1223;
const SECURITY_BUILTIN_DOMAIN_RID: u32 = 0x0000_0020;
const DOMAIN_ALIAS_RID_ADMINS: u32 = 0x0000_0220;
const SETUP_EXE_FILENAME: &str = "sprite-windows-sandbox-setup.exe";
...
pub fn sandbox_dir(sprite_home: &Path) -> PathBuf {
    sprite_home.join(".sandbox")
}

pub fn sandbox_bin_dir(sprite_home: &Path) -> PathBuf {
    sprite_home.join(".sandbox-bin")
}

pub fn sandbox_secrets_dir(sprite_home: &Path) -> PathBuf {
    sprite_home.join(".sandbox-secrets")
}

pub fn setup_marker_path(sprite_home: &Path) -> PathBuf {
    sandbox_dir(sprite_home).join("setup_marker.json")
}

pub fn sandbox_users_path(sprite_home: &Path) -> PathBuf {
    sandbox_secrets_dir(sprite_home).join("sandbox_users.json")
}
...
pub fn run_setup_refresh_with_extra_read_roots(
    ...
    let mut read_roots = gather_read_roots(command_cwd, &permissions, env_map, sprite_home);
...
fn run_setup_refresh_inner(
    ...
    let payload = ElevationPayload {
        ...
        sprite_home: request.sprite_home.to_path_buf(),
    ...
    let sbx_dir = sandbox_dir(request.sprite_home);
    let log_path = current_log_file_path(&sbx_dir);
    let cleared_report = match clear_setup_error_report(request.sprite_home) {
        Ok(()) => true,
        Err(err) => {
            log_note(
    ...
    let cwd = std::env::current_dir().unwrap_or_else(|_| request.sprite_home.to_path_buf());
    log_note(
        &format!(
    ...
    if let Err(err) = clear_setup_error_report(request.sprite_home) {
        log_note(
...
fn gather_helper_read_roots(sprite_home: &Path) -> Vec<PathBuf> {
    let helper_dir = helper_bin_dir(sprite_home);
...
fn gather_full_read_roots_for_permissions(
    ...
    let mut roots = gather_helper_read_roots(sprite_home);
    roots.extend(
...
pub(crate) fn effective_write_roots_for_permissions(
    ...
    filter_sensitive_write_roots(write_roots, sprite_home)
}
...
fn report_helper_failure(
    ...
    match read_setup_error_report(sprite_home) {
...
fn verify_setup_completed(sprite_home: &Path) -> Result<()> {
    if sandbox_setup_is_complete(sprite_home) {
        Ok(())
    } else {
        Err(failure(
...
fn run_setup_exe(
    ...
    let cleared_report = match clear_setup_error_report(sprite_home) {
        Ok(()) => true,
        Err(err) => {
            log_note(
                &format!(
    ...
    if !needs_elevation {
        ...
        verify_setup_completed(sprite_home)?;
        if let Err(err) = clear_setup_error_report(sprite_home) {
            log_note(
                &format!(
...
pub fn run_elevated_setup(
    ...
    run_setup_exe(&payload, needs_elevation, request.sprite_home)
}

pub fn run_elevated_provisioning_setup(sprite_home: &Path, real_user: &str) -> Result<()> {
    let sbx_dir = sandbox_dir(sprite_home);
    std::fs::create_dir_all(&sbx_dir).map_err(|err| {
        failure(
            SetupErrorCode::OrchestratorSandboxDirCreateFailed,
            format!("failed to create sandbox dir {}: {err}", sbx_dir.display()),
    ...
    let payload = ElevationPayload {
        ...
        command_cwd: sprite_home.to_path_buf(),
        read_roots: Vec::new(),
        write_roots: Vec::new(),
    ...
    run_setup_exe(&payload, /*needs_elevation*/ false, sprite_home)
}
...
fn build_payload_roots(
    ...
    let mut read_roots = if let Some(roots) = overrides.read_roots.as_deref() {
        ...
        let mut read_roots = gather_helper_read_roots(request.sprite_home);
...
fn filter_sensitive_write_roots(mut roots: Vec<PathBuf>, sprite_home: &Path) -> Vec<PathBuf> {
    // Never grant capability write access to SPRITE_HOME or anything under SPRITE_HOME/.sandbox,
    // SPRITE_HOME/.sandbox-bin, or SPRITE_HOME/.sandbox-secrets. These locations contain sandbox
    // control/state and helper binaries and must remain tamper-resistant.
    let sprite_home_key = canonical_path_key(sprite_home);
    let sbx_dir_key = canonical_path_key(&sandbox_dir(sprite_home));
    let sbx_dir_prefix = format!("{}/", sbx_dir_key.trim_end_matches('/'));
    let sbx_bin_dir_key = canonical_path_key(&sandbox_bin_dir(sprite_home));
    let sbx_bin_dir_prefix = format!("{}/", sbx_bin_dir_key.trim_end_matches('/'));
    let secrets_dir_key = canonical_path_key(&sandbox_secrets_dir(sprite_home));
...
mod tests {
    ...
    fn setup_completion_requires_ready_artifacts() {
        let sprite_home = TempDir::new().expect("tempdir");
        let err = verify_setup_completed(sprite_home.path())
    ...
    fn report_helper_failure_uses_setup_error_report_when_clear_succeeded() {
        ...
        let sprite_home = tmp.path().join("sprite-home");
    ...
    fn setup_exe_lookup_checks_package_resource_dir_for_bin_exe() {
        ...
        let exe = bin_dir.join("sprite.exe");
        let setup_exe = resources_dir.join("sprite-windows-sandbox-setup.exe");
    ...
    #[test]
    fn expanded_write_roots_still_drop_protected_sprite_home() {
        let tmp = TempDir::new().expect("tempdir");
        let user_profile = tmp.path().join("user-profile");
        let sprite_home = user_profile.join("SpriteHome");
        let documents = user_profile.join("Documents");
        fs::create_dir_all(&sprite_home).expect("create sprite home");
        fs::create_dir_all(&documents).expect("create documents");
        ...
        let roots = super::filter_sensitive_write_roots(roots, &sprite_home);
    ...
    fn gather_read_roots_includes_helper_bin_dir() {
        ...
        let roots = gather_read_roots(&command_cwd, &permissions, &HashMap::new(), &sprite_home);
        let expected =
            dunce::canonicalize(helper_bin_dir(&sprite_home)).expect("canonical helper dir");
    ...
    fn effective_write_roots_match_payload_filtering_for_overrides() {
        ...
        let sandbox_root = super::sandbox_dir(&sprite_home);
        fs::create_dir_all(&sprite_home).expect("create sprite home");
        fs::create_dir_all(&command_cwd).expect("create workspace");
        ...
        let forbidden_sprite_home =
            dunce::canonicalize(&sprite_home).expect("canonical sprite home");
        ...
        assert!(!effective_write_roots.contains(&forbidden_sprite_home));
    ...
    fn payload_deny_write_paths_merge_explicit_and_protected_children() {
        ...
        let extra_sprite = extra_write_root.join(".sprite");
        let explicit_deny = tmp.path().join("explicit-deny");
        fs::create_dir_all(&command_git).expect("create command .git");
        fs::create_dir_all(&extra_sprite).expect("create extra .sprite");
        let writable_roots = vec![
        ...
        assert_eq!(
            [
                ...
                dunce::canonicalize(&extra_sprite).expect("canonical extra .sprite"),
                explicit_deny,
            ]
            .into_iter()
