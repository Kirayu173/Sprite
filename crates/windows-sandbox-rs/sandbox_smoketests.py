...
# Run a suite of smoke tests against the Windows sandbox via the Sprite CLI
...
def _resolve_sprite_cmd() -> List[str]:
    """Resolve the Sprite CLI to invoke `sprite sandbox windows`.

    Prefer local builds (debug first), then fall back to PATH.
    Returns the argv prefix to run Sprite.
    """
    ...
    candidates = [
        ws_root / "target" / "debug" / "sprite.exe",
        ws_root / "target" / "release" / "sprite.exe",
    ]
    if cargo_target:
        cargo_base = Path(cargo_target)
        candidates.extend([
            cargo_base / "debug" / "sprite.exe",
            cargo_base / "release" / "sprite.exe",
        ])
    ...
    raise FileNotFoundError(
        "Sprite CLI not found. Build it first, e.g.\n"
        "  cargo build -p sprite-cli --release\n"
        "or for debug:\n"
        "  cargo build -p sprite-cli\n"
    )

SPRITE_CMD = _resolve_sprite_cmd()
print(SPRITE_CMD)
TIMEOUT_SEC = 20
...
def run_sbx(
    ...
    argv = [*SPRITE_CMD, "sandbox", "windows", *policy_flags, *overrides, "--", *cmd_argv]
    print(cmd_argv)
...
def main() -> int:
    ...
    if have("curl"):
        with start_loopback_proxy_fixture() as (target_port, proxy_port):
            ...
            rc_direct, _out_direct, err_direct = run_sbx(
                ...
                direct_cmd,
                WS_ROOT,
                env_extra={"SPRITE_HOME": str(proxy_home)},
            )
    ...
    rc, out, err = run_sbx("workspace-write", ["cmd", "/c", "echo hi > \\\\.\\pipe\\sprite_testpipe"], WS_ROOT)
    ...
    # 34. WS: policy tamper (.sprite artifacts) denied
    sprite_home = Path(os.environ["USERPROFILE"]) / ".sprite"
    cap_sid_target = sprite_home / "cap_sid"
    rc, out, err = run_sbx(
        "workspace-write",
        ["cmd", "/c", f"echo tamper > \"{cap_sid_target}\""],
        WS_ROOT,
    )
    rc2, out2, err2 = run_sbx("workspace-write", ["cmd", "/c", "echo tamper > .sprite\\policy.json"], WS_ROOT)
    add("WS: .sprite cap_sid tamper denied", rc != 0, f"rc={rc}, err={err}")
    add("WS: .sprite policy tamper denied", rc2 != 0, f"rc={rc2}, err={err2}")
    ...
    if make_symlink(fake_root, Path("C:/")):
        rc, out, err = run_sbx("workspace-write", ["cmd", "/c", "echo owned > sprite_escape.txt"], fake_root)
    ...
    rc, out, err = run_sbx(
        ...
        [
            ...
            "-NoProfile",
            "-Command",
            "Start-Process 'https://sprite-invalid.local/smoke'",
        ],
        WS_ROOT,
    )
    add(
