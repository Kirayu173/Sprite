...
use runtime_protocol::items::parse_hook_prompt_message;
...
use runtime_protocol::protocol::ContextCompactedEvent;
use runtime_protocol::protocol::DynamicToolCallResponseEvent;
use runtime_protocol::protocol::ErrorEvent;
...
use runtime_protocol::protocol::ItemStartedEvent;
use runtime_protocol::protocol::McpToolCallBeginEvent;
...
use runtime_protocol::protocol::ReviewOutputEvent;
...
use runtime_protocol::protocol::ViewImageToolCallEvent;
use runtime_protocol::protocol::WebSearchBeginEvent;
...
impl ThreadHistoryBuilder {
    ...
    /// See the rollout persistence policy in the upstream runtime implementation.
    ...
    fn handle_response_item(&mut self, item: &runtime_protocol::models::ResponseItem) {
        let runtime_protocol::models::ResponseItem::Message {
            role, content, id, ..
        } = item
        else {
            return;
    ...
    fn handle_item_started(&mut self, payload: &ItemStartedEvent) {
        match &payload.item {
            runtime_protocol::items::TurnItem::Plan(plan) => {
            ...
            runtime_protocol::items::TurnItem::UserMessage(_)
            | runtime_protocol::items::TurnItem::HookPrompt(_)
            | runtime_protocol::items::TurnItem::AgentMessage(_)
            | runtime_protocol::items::TurnItem::Reasoning(_)
            | runtime_protocol::items::TurnItem::WebSearch(_)
            | runtime_protocol::items::TurnItem::ImageView(_)
            | runtime_protocol::items::TurnItem::ImageGeneration(_)
            | runtime_protocol::items::TurnItem::FileChange(_)
            | runtime_protocol::items::TurnItem::McpToolCall(_)
            | runtime_protocol::items::TurnItem::ContextCompaction(_) => {}
        }
    }
    ...
    fn handle_dynamic_tool_call_request(
        &mut self,
        payload: &runtime_protocol::dynamic_tools::DynamicToolCallRequest,
    ) {
    ...
    fn handle_collab_agent_spawn_begin(
        &mut self,
        payload: &runtime_protocol::protocol::CollabAgentSpawnBeginEvent,
    ) {
    ...
    fn handle_collab_agent_spawn_end(
        &mut self,
        payload: &runtime_protocol::protocol::CollabAgentSpawnEndEvent,
    ) {
    ...
    fn handle_collab_agent_interaction_begin(
        &mut self,
        payload: &runtime_protocol::protocol::CollabAgentInteractionBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_agent_interaction_end(
        &mut self,
        payload: &runtime_protocol::protocol::CollabAgentInteractionEndEvent,
    ) {
        let status = match &payload.status {
            AgentStatus::Errored(_) | AgentStatus::NotFound => CollabAgentToolCallStatus::Failed,
    ...
    fn handle_collab_waiting_begin(
        &mut self,
        payload: &runtime_protocol::protocol::CollabWaitingBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_waiting_end(
        &mut self,
        payload: &runtime_protocol::protocol::CollabWaitingEndEvent,
    ) {
    ...
    fn handle_collab_close_begin(
        &mut self,
        payload: &runtime_protocol::protocol::CollabCloseBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_resume_begin(
        &mut self,
        payload: &runtime_protocol::protocol::CollabResumeBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_resume_end(
        &mut self,
        payload: &runtime_protocol::protocol::CollabResumeEndEvent,
    ) {
        let status = match &payload.status {
            AgentStatus::Errored(_) | AgentStatus::NotFound => CollabAgentToolCallStatus::Failed,
            _ => CollabAgentToolCallStatus::Completed,
    ...
    fn handle_entered_review_mode(&mut self, payload: &runtime_protocol::protocol::ReviewRequest) {
        let review = payload
            .user_facing_hint
            .clone()
    ...
    fn handle_exited_review_mode(
        &mut self,
        payload: &runtime_protocol::protocol::ExitedReviewModeEvent,
    ) {
        let review = payload
            .review_output
            .as_ref()
    ...
    fn handle_error(&mut self, payload: &ErrorEvent) {
        ...
        turn.error = Some(V2TurnError {
            ...
            error_info: payload.error_info.clone().map(Into::into),
...
fn convert_dynamic_tool_content_items(
    items: &[runtime_protocol::dynamic_tools::DynamicToolCallOutputContentItem],
...
mod tests {
    ...
    use runtime_protocol::items::UserMessageItem as CoreUserMessageItem;
    use runtime_protocol::items::build_hook_prompt_message;
    use runtime_protocol::mcp::CallToolResult;
    use runtime_protocol::models::ImageDetail;
    use runtime_protocol::models::MessagePhase as CoreMessagePhase;
    use runtime_protocol::models::WebSearchAction as CoreWebSearchAction;
    use runtime_protocol::parse_command::ParsedCommand;
    use runtime_protocol::protocol::AgentMessageEvent;
    use runtime_protocol::protocol::AgentReasoningEvent;
    use runtime_protocol::protocol::AgentReasoningRawContentEvent;
    use runtime_protocol::protocol::ApplyPatchApprovalRequestEvent;
    use runtime_protocol::protocol::CompactedItem;
    use runtime_protocol::protocol::DynamicToolCallResponseEvent;
    use runtime_protocol::protocol::ExecCommandEndEvent;
    use runtime_protocol::protocol::ExecCommandSource;
    use runtime_protocol::protocol::ItemStartedEvent;
        ...
        let events = vec![
            ...
            EventMsg::ItemStarted(ItemStartedEvent {
                ...
                item: CoreTurnItem::UserMessage(CoreUserMessageItem {
                    ...
                    content: vec![runtime_protocol::user_input::UserInput::Text {
                        text: "hello".into(),
                        text_elements: Vec::new(),
                    }],
                }),
    ...
    fn reconstructs_dynamic_tool_items_from_request_and_response_events() {
        let events = vec![
            ...
            EventMsg::DynamicToolCallRequest(
                runtime_protocol::dynamic_tools::DynamicToolCallRequest {
                    call_id: "dyn-1".into(),
                    turn_id: "turn-1".into(),
                    started_at_ms: 0,
                    namespace: Some("sprite_app".into()),
        ...
        let events = vec![
            ...
            EventMsg::PatchApplyEnd(PatchApplyEndEvent {
                ...
                changes: [(
                    ...
                    runtime_protocol::protocol::FileChange::Add {
                        content: "hello\n".into(),
                    },
                )]
                .into_iter()
    ...
    fn reconstructs_collab_resume_end_item() {
        let events = vec![
            ...
            EventMsg::CollabResumeEnd(runtime_protocol::protocol::CollabResumeEndEvent {
    ...
    fn reconstructs_collab_spawn_end_item_with_model_metadata() {
        ...
        let events = vec![
            ...
            EventMsg::CollabAgentSpawnEnd(runtime_protocol::protocol::CollabAgentSpawnEndEvent {
    ...
    fn reconstructs_interrupted_send_input_as_completed_collab_call() {
        ...
        let events = vec![
            ...
            EventMsg::CollabAgentInteractionBegin(
                runtime_protocol::protocol::CollabAgentInteractionBeginEvent {
                    call_id: "send-1".into(),
                    started_at_ms: 0,
            ...
            EventMsg::CollabAgentInteractionEnd(
                runtime_protocol::protocol::CollabAgentInteractionEndEvent {
                    call_id: "send-1".into(),
                    completed_at_ms: 0,
                    sender_thread_id: sender,
                    receiver_thread_id: receiver,
    ...
    fn rollback_failed_error_does_not_mark_turn_failed() {
        let events = vec![
            ...
            EventMsg::Error(ErrorEvent {
                ...
                error_info: Some(RuntimeErrorInfo::ThreadRollbackFailed),
            }),
        ];
    ...
    fn out_of_turn_error_does_not_create_or_fail_a_turn() {
        let events = vec![
            ...
            EventMsg::Error(ErrorEvent {
                ...
                error_info: Some(RuntimeErrorInfo::BadRequest),
            }),
        ];
    ...
    fn error_then_turn_complete_preserves_failed_status() {
        let events = vec![
            ...
            EventMsg::Error(ErrorEvent {
                ...
                error_info: Some(RuntimeErrorInfo::ResponseStreamDisconnected {
        ...
        assert_eq!(
            ...
            Some(TurnError {
                message: "stream failure".into(),
                error_info: Some(
                    crate::protocol::v2::RuntimeErrorInfo::ResponseStreamDisconnected {
                        http_status_code: Some(502),
                    }
                ),
                additional_details: None,
    ...
    fn ignores_plain_user_response_items_in_rollout_replay() {
        let items = vec![
            ...
            RolloutItem::ResponseItem(runtime_protocol::models::ResponseItem::Message {
                id: Some("msg-1".into()),
                role: "user".into(),
                content: vec![runtime_protocol::models::ContentItem::InputText {
