...
use runtime_protocol::items::FileChangeItem;
use runtime_protocol::items::ImageViewItem;
use runtime_protocol::items::McpToolCallItem;
use runtime_protocol::items::McpToolCallStatus as CoreMcpToolCallStatus;
use runtime_protocol::items::ReasoningItem;
use runtime_protocol::items::TurnItem;
use runtime_protocol::items::UserMessageItem;
use runtime_protocol::items::WebSearchItem;
use runtime_protocol::mcp::CallToolResult;
use runtime_protocol::mcp::McpServerInfo;
...
use runtime_protocol::models::AdditionalPermissionProfile as CoreAdditionalPermissionProfile;
use runtime_protocol::models::BUILT_IN_PERMISSION_PROFILE_WORKSPACE;
use runtime_protocol::models::FileSystemPermissions as CoreFileSystemPermissions;
use runtime_protocol::models::ImageDetail;
use runtime_protocol::models::MessagePhase;
use runtime_protocol::models::NetworkPermissions as CoreNetworkPermissions;
use runtime_protocol::models::WebSearchAction as CoreWebSearchAction;
use runtime_protocol::permissions::FileSystemAccessMode as CoreFileSystemAccessMode;
use runtime_protocol::permissions::FileSystemPath as CoreFileSystemPath;
use runtime_protocol::permissions::FileSystemSandboxEntry as CoreFileSystemSandboxEntry;
use runtime_protocol::permissions::FileSystemSpecialPath as CoreFileSystemSpecialPath;
        ...
        ExternalAgentConfigMigrationItem {
            ...
            details: Some(MigrationDetails {
                plugins: vec![PluginsMigration {
                    catalog_name: "team-catalog".to_string(),
...
fn sandbox_policy_round_trips_external_sandbox_network_access() {
    ...
    assert_eq!(
        core_policy,
        runtime_protocol::protocol::SandboxPolicy::ExternalSandbox {
...
fn sandbox_policy_round_trips_read_only_network_access() {
    ...
    assert_eq!(
        core_policy,
        runtime_protocol::protocol::SandboxPolicy::ReadOnly {
            network_access: true,
        }
    );
...
fn sandbox_policy_round_trips_workspace_write_access() {
    ...
    assert_eq!(
        core_policy,
        runtime_protocol::protocol::SandboxPolicy::WorkspaceWrite {
            writable_roots: vec![],
            network_access: true,
            exclude_tmpdir_env_var: false,
            exclude_slash_tmp: false,
...
fn network_requirements_deserializes_legacy_fields() {
    ...
    assert_eq!(
        ...
        NetworkRequirements {
            ...
            allowed_domains: Some(vec!["api.sprite.dev".to_string()]),
...
fn network_requirements_serializes_canonical_and_legacy_fields() {
    let requirements = NetworkRequirements {
        ...
        domains: Some(BTreeMap::from([
            ("api.sprite.dev".to_string(), NetworkDomainPermission::Allow),
            (
...
fn core_turn_item_into_thread_item_converts_supported_variants() {
    let user_item = TurnItem::UserMessage(UserMessageItem {
        ...
        content: vec![
            ...
            CoreUserInput::Skill {
                ...
                path: PathBuf::from("/repo/.sprite/skills/skill-creator/SKILL.md"),
            },
    ...
    let file_change_item = TurnItem::FileChange(FileChangeItem {
        ...
        .into_iter()
        .collect(),
        status: Some(runtime_protocol::protocol::PatchApplyStatus::Completed),
        auto_approved: None,
    ...
    let mcp_tool_call_item = TurnItem::McpToolCall(McpToolCallItem {
        ...
        mcp_app_resource_uri: Some("app://integration".to_string()),
        integration_id: Some("sample@test".to_string()),
...
#[test]
fn plugin_source_serializes_local_and_git_variants() {
    ...
    assert_eq!(
        serde_json::to_value(PluginSource::Git {
            url: "https://github.com/sprite/example.git".to_string(),
        ...
        json!({
            "type": "git",
            "url": "https://github.com/sprite/example.git",
...
fn plugin_read_params_serialization_uses_install_source_fields() {
    let catalog_path = if cfg!(windows) {
        r"C:\plugins\catalog.json"
    } else {
        "/plugins/catalog.json"
    };
    let catalog_path = AbsolutePathBuf::try_from(PathBuf::from(catalog_path)).unwrap();
    let catalog_path_json = catalog_path.as_path().display().to_string();
    assert_eq!(
...
fn plugin_summary_defaults_missing_availability_to_available() {
    let summary: PluginSummary = serde_json::from_value(json!({
        ...
        "source": {
            "type": "git",
            "url": "https://example.com/gmail.git",
            "path": null,
            "refName": null,
            "sha": null
        },
...
fn plugin_uninstall_params_serialization_omits_force_remote_sync() {
    assert_eq!(
        serde_json::to_value(PluginUninstallParams {
            plugin_id: "gmail@sprite-curated".to_string(),
        })
        .unwrap(),
        json!({
            "pluginId": "gmail@sprite-curated",
        }),
    );
...
#[test]
fn error_info_serializes_active_turn_not_steerable_turn_kind_in_camel_case() {
    let value = RuntimeErrorInfo::ActiveTurnNotSteerable {
...
fn thread_settings_update_params_preserve_field_level_experimental_gates() {
    ...
    let collaboration_mode = ThreadSettingsUpdateParams {
        thread_id: "thread_123".to_string(),
        collaboration_mode: Some(runtime_protocol::config_types::CollaborationMode {
            mode: runtime_protocol::config_types::ModeKind::Plan,
