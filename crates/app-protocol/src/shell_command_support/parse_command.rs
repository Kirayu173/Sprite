use crate::shell_command_support::bash::extract_bash_command;
use crate::shell_command_support::bash::try_parse_shell;
use crate::shell_command_support::bash::try_parse_word_only_commands_sequence;
use crate::shell_command_support::powershell::extract_powershell_command;
...
/// The easiest way to iterate is to add unit tests and have the agent fix the implementation.
...
/// Tests are at the top to encourage using TDD + the agent to fix the implementation.
mod tests {
    use super::*;
    ...
    fn supports_grep_recursive_current_dir() {
        assert_parsed(
            &vec_str(&["grep", "-R", "SPRITE_SANDBOX_ENV_VAR", "-n", "."]),
            vec![ParsedCommand::Search {
                cmd: "grep -R SPRITE_SANDBOX_ENV_VAR -n .".to_string(),
                query: Some("SPRITE_SANDBOX_ENV_VAR".to_string()),
                path: Some(".".to_string()),
            }],
        );
    }
    ...
    fn supports_grep_recursive_specific_file() {
        assert_parsed(
            &vec_str(&[
                "grep",
                "-R",
                "SPRITE_SANDBOX_ENV_VAR",
                "-n",
                "core/src/spawn.rs",
            ]),
            vec![ParsedCommand::Search {
                cmd: "grep -R SPRITE_SANDBOX_ENV_VAR -n core/src/spawn.rs".to_string(),
                query: Some("SPRITE_SANDBOX_ENV_VAR".to_string()),
    ...
    fn supports_cd_and_rg_files() {
        assert_parsed(
            &shlex_split_safe("cd sprite-rs && rg --files"),
            vec![ParsedCommand::ListFiles {
                cmd: "rg --files".to_string(),
                path: None,
            }],
    ...
    fn supports_single_string_script_with_cd_and_pipe() {
        let inner =
            r#"cd /Users/pakrym/code/sprite && rg -n "sprite_api" sprite-rs -S | head -n 50"#;
        assert_parsed(
            &vec_str(&["bash", "-lc", inner]),
            vec![ParsedCommand::Search {
                cmd: "rg -n sprite_api sprite-rs -S".to_string(),
                query: Some("sprite_api".to_string()),
                path: Some("sprite-rs".to_string()),
            }],
        );
    }
...
pub fn parse_command_impl(command: &[String]) -> Vec<ParsedCommand> {
    ...
    let parts = if contains_command_separators(&normalized) {
        split_on_command_separators(&normalized)
    } else {
        vec![normalized]
    };
...
fn sed_read_path(args: &[String]) -> Option<String> {
    let args_before_separator = trim_at_command_separator(args);
    if !args_before_separator.iter().any(|arg| arg == "-n") {
        return None;
    }
    let mut has_range_script = false;
    let mut i = 0;
    while i < args_before_separator.len() {
        let arg = &args_before_separator[i];
        if matches!(arg.as_str(), "-e" | "--expression") {
            if is_valid_sed_n_arg(args_before_separator.get(i + 1).map(String::as_str)) {
    ...
    if !has_range_script {
        has_range_script = args_before_separator
            .iter()
...
fn contains_command_separators(tokens: &[String]) -> bool {
    tokens
        .iter()
        .any(|t| t == "&&" || t == "||" || t == "|" || t == ";")
}

fn split_on_command_separators(tokens: &[String]) -> Vec<Vec<String>> {
...
fn trim_at_command_separator(tokens: &[String]) -> Vec<String> {
    let idx = tokens
        .iter()
        .position(|t| t == "|" || t == "&&" || t == "||" || t == ";")
...
fn parse_grep_like(main_cmd: &[String], args: &[String]) -> ParsedCommand {
    ...
    let mut after_double_dash = false;
    let mut iter = args_before_separator.iter().peekable();
...
fn awk_data_file_operand(args: &[String]) -> Option<String> {
    ...
    let args_before_separator = trim_at_command_separator(args);
    let has_script_file = args_before_separator
        .iter()
...
fn python_walks_files(args: &[String]) -> bool {
    let args_before_separator = trim_at_command_separator(args);
    let mut iter = args_before_separator.iter();
    while let Some(arg) = iter.next() {
        if arg == "-c"
...
fn parse_fd_query_and_path(tail: &[String]) -> (Option<String>, Option<String>) {
    let args_before_separator = trim_at_command_separator(tail);
...
fn parse_find_query_and_path(tail: &[String]) -> (Option<String>, Option<String>) {
    ...
    for a in &args_before_separator {
    ...
    let mut i = 0;
    while i < args_before_separator.len() {
        let a = &args_before_separator[i];
        if a == "-name" || a == "-iname" || a == "-path" || a == "-regex" {
            if i + 1 < args_before_separator.len() {
                query = Some(args_before_separator[i + 1].clone());
            }
            break;
        }
        i += 1;
...
fn parse_shell_lc_commands(original: &[String]) -> Option<Vec<ParsedCommand>> {
        ...
        && !all_commands.is_empty()
    {
        ...
        if commands.len() == 1 {
            ...
            // no command separators in the original script. For pipeline commands (e.g.
            // `rg --files | sed -n`), keep only the primary command.
            let had_separators = had_multiple_commands
                || script_tokens
                    .iter()
                    .any(|t| t == "|" || t == "&&" || t == "||" || t == ";");
            commands = commands
...
fn summarize_main_tokens(main_cmd: &[String]) -> ParsedCommand {
    match main_cmd.split_first() {
        ...
        Some((head, tail)) if head == "rg" || head == "rga" || head == "ripgrep-all" => {
            let args_before_separator = trim_at_command_separator(tail);
            let has_files_flag = args_before_separator.iter().any(|a| a == "--files");
            let candidates = skip_flag_values(
                &args_before_separator,
                &[
                    "-g",
