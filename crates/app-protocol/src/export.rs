...
pub fn generate_json_with_experimental(out_dir: &Path, experimental_api: bool) -> Result<()> {
    ...
    write_pretty_json(out_dir.join("app_protocol.schemas.json"), &bundle)?;
    let flat_v2_bundle = build_flat_v2_schema(&bundle)?;
    write_pretty_json(
        out_dir.join("app_protocol.v2.schemas.json"),
        &flat_v2_bundle,
    )?;
...
fn build_schema_bundle(schemas: Vec<GeneratedSchema>) -> Result<Value> {
    ...
    root.insert(
        "title".to_string(),
        Value::String("AppServerProtocol".into()),
    );
...
mod tests {
    ...
    fn generated_ts_optional_nullable_fields_only_in_params() -> Result<()> {
        ...
        assert_eq!(
            fixture_tree.contains_key(Path::new("v2/LegacyRemoteClient.ts")),
            false
        );
        assert_eq!(
            fixture_tree.contains_key(Path::new("v2/LegacyRemoteClientsListOrder.ts")),
            false
        );
    ...
    fn stable_schema_filter_removes_mock_thread_start_field() -> Result<()> {
        let output_dir = std::env::temp_dir().join(format!("sprite_schema_{}", Uuid::now_v7()));
    ...
    fn build_flat_v2_schema_keeps_shared_root_schemas_and_dependencies() -> Result<()> {
        ...
        assert_eq!(
            flat_bundle["title"],
            serde_json::json!("AppServerProtocolV2")
        );
    ...
    fn experimental_type_fields_ts_filter_handles_interface_shape() -> Result<()> {
        let output_dir = std::env::temp_dir().join(format!("sprite_ts_filter_{}", Uuid::now_v7()));
...
"#;
    ...
    fn generate_json_filters_experimental_fields_and_methods() -> Result<()> {
        ...
        let bundle_json = fs::read_to_string(output_dir.join("app_protocol.schemas.json"))?;
        ...
        let flat_v2_bundle_json =
            fs::read_to_string(output_dir.join("app_protocol.v2.schemas.json"))?;
        ...
        assert_eq!(flat_v2_bundle_json.contains("LegacyRemoteClient"), false);
        assert_eq!(
            flat_v2_bundle_json.contains("LegacyRemoteClientsListOrder"),
            false
        );
        assert_eq!(flat_v2_bundle_json.contains("#/definitions/v2/"), false);
        assert_eq!(
            flat_v2_bundle_json.contains("\"title\": \"AppServerProtocolV2\""),
            true
        );
        let flat_v2_bundle = read_json_value(&output_dir.join("app_protocol.v2.schemas.json"))?;
    ...
    #[test]
    fn generate_json_excludes_remote_control_methods_even_with_experimental_api() -> Result<()> {
        let output_dir = std::env::temp_dir().join(format!("sprite_schema_{}", Uuid::now_v7()));
        fs::create_dir(&output_dir)?;
