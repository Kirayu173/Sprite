...
/// LoaderOverrides customizes config loading inputs, primarily for tests.
#[derive(Debug, Default, Clone)]
...
impl LoaderOverrides {
    /// Returns overrides that ignore host-managed requirements.
    ///
    /// This is intended for tests that should load only repo-controlled config fixtures.
    pub fn without_host_requirements_for_tests() -> Self {
        let base = std::env::temp_dir().join("sprite-config-tests");
        Self {
    ...
    pub fn user_config_path(&self, sprite_home: &Path) -> std::io::Result<AbsolutePathBuf> {
...
impl ConfigLayerEntry {
    ...
    // Get the `.sprite/` folder associated with this config layer, if any.
    pub fn config_folder(&self) -> Option<AbsolutePathBuf> {
        match &self.name {
            ConfigLayerSource::System { file } => file.parent(),
            ConfigLayerSource::User { file, .. } => file.parent(),
            ConfigLayerSource::Project { project_config_dir } => Some(project_config_dir.clone()),
            ConfigLayerSource::SessionFlags => None,
        }
    }

    /// Returns the `.sprite/` folder that should be used for hook declarations.
    ///
...
pub struct ConfigLayerStack {
    ...
    /// the base `$SPRITE_HOME/config.toml` layer followed by the profile override
    ...
    /// Raw requirements data as loaded from requirements.toml. This preserves
    /// the original allow-lists so they can be surfaced via APIs.
...
impl ConfigLayerStack {
    ...
    /// the base `$SPRITE_HOME/config.toml` layer because the active layer is the
...
fn verify_layer_ordering(layers: &[ConfigLayerEntry]) -> std::io::Result<Option<usize>> {
    ...
    let mut previous_project_config_dir: Option<&AbsolutePathBuf> = None;
    for (index, layer) in layers.iter().enumerate() {
        ...
        if let ConfigLayerSource::Project {
            project_config_dir: current_project_config_dir,
        } = &layer.name
        {
            if let Some(previous) = previous_project_config_dir {
                ...
                if previous == current_project_config_dir
                    || !current_project_config_dir
                        .as_path()
                        .ancestors()
            ...
            previous_project_config_dir = Some(current_project_config_dir);
        }
    }
