//! Schema-heavy configuration TOML types used by Sprite.
...
use utils_path::normalize_for_path_comparison;
...
/// Base config deserialized from ~/.sprite/config.toml.
...
pub struct ConfigToml {
    ...
    /// DISCOURAGED from using this field, as deviating from the runtime
    /// instructions will likely degrade model performance.
    pub model_instructions_file: Option<AbsolutePathBuf>,

    /// Compact prompt used for history compaction.
    pub compact_prompt: Option<String>,

    /// Definition for MCP servers that Sprite can reach out to for tool calls.
    #[serde(default)]
    ...
    /// Directory where Sprite stores the SQLite state DB.
    /// Defaults to `$SPRITE_SQLITE_HOME` when set. Otherwise uses `$SPRITE_HOME`.
    pub sqlite_home: Option<AbsolutePathBuf>,

    /// Directory where Sprite writes log files. Setting this value explicitly
    /// also enables the TUI text log in this directory.
    /// Defaults to `$SPRITE_HOME/log`.
    ...
    /// Default model slug to use for built-in local providers when `model` is
    /// unset. Prefer this over relying on baked-in local model fallbacks.
    pub local_provider_default_model: Option<String>,
    ...
    /// directories for `.sprite` folders. Defaults to [".git"] when unset.
    #[serde(default)]
