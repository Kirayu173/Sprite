...
fn layer(id: &str, name: &str, contents: &str) -> RequirementsLayerEntry {
    let file = if cfg!(windows) {
        format!("C:\\sprite\\requirements\\{id}.toml")
    } else {
        format!("/etc/sprite/requirements/{id}.toml")
    };
    let _ = name;
    RequirementsLayerEntry::from_toml(
        RequirementSource::SystemRequirementsToml {
            file: AbsolutePathBuf::from_absolute_path(file).expect("absolute path"),
        },
        contents,
    )
}
...
#[test]
fn composition_strategy_applies_to_regular_layers() {
    let high_requirements_file = if cfg!(windows) {
        "C:\\requirements-high.toml"
    } else {
        "/etc/sprite/requirements-high.toml"
    };
    let high_source = RequirementSource::SystemRequirementsToml {
        file: AbsolutePathBuf::from_absolute_path(high_requirements_file).expect("absolute path"),
    };
    let system_file = if cfg!(windows) {
        "C:\\requirements.toml"
    } else {
        "/etc/sprite/requirements.toml"
    };
...
"#
    ...
    assert_eq!(
        ...
        Some(Sourced::new(vec![AskForApproval::Never], high_source))
    );
}

#[test]
fn single_regular_layer_keeps_system_source() {
    let expected_source = RequirementSource::SystemRequirementsToml {
        file: AbsolutePathBuf::from_absolute_path(if cfg!(windows) {
            "C:\\sprite\\requirements\\req_1.toml"
        } else {
            "/etc/sprite/requirements/req_1.toml"
        })
...
"#,
    ...
    assert_eq!(
        ...
        Some(Sourced::new(/*value*/ true, expected_source,))
    );
}
...
fn merged_table_source_is_composite_in_priority_order() {
    let high_source = RequirementSource::SystemRequirementsToml {
        file: AbsolutePathBuf::from_absolute_path(if cfg!(windows) {
            "C:\\sprite\\requirements\\req_high.toml"
        } else {
            "/etc/sprite/requirements/req_high.toml"
        })
        .expect("absolute path"),
    };
    let low_source = RequirementSource::SystemRequirementsToml {
        file: AbsolutePathBuf::from_absolute_path(if cfg!(windows) {
            "C:\\sprite\\requirements\\req_low.toml"
        } else {
            "/etc/sprite/requirements/req_low.toml"
        })
        .expect("absolute path"),
    };
    let composed = compose_requirements_for_hostname(
...
"#,
    ...
    assert!(err.to_string().contains("req_high.toml"));
    assert!(err.to_string().contains("req_low.toml"));
}
...
fn parse_error_names_layer() {
    ...
    assert!(err.to_string().contains("req_bad.toml"));
