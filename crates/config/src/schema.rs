...
use features::legacy_feature_keys;
...
pub fn features_schema(schema_gen: &mut SchemaGenerator) -> Schema {
    ...
    for feature in FEATURES {
        if feature.id == features::Feature::Artifact {
            continue;
        }
        if feature.id == features::Feature::CodeMode {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<features::FeatureToml<features::CodeModeConfigToml>>(),
            );
            continue;
        }
        if feature.id == features::Feature::NetworkProxy {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen
                    .subschema_for::<features::FeatureToml<features::NetworkProxyConfigToml>>(),
            );
            continue;
        }
        validation
