...
mod tests {
    ...
    use anyhow::Result;
    use execpolicy::Decision;
    use execpolicy::Evaluation;
    use execpolicy::RuleMatch;
    ...
    fn system_requirements_source_for_test() -> RequirementSource {
        RequirementSource::SystemRequirementsToml {
            file: system_requirements_toml_file_for_test().expect("requirements path"),
        }
    }

    #[test]
    fn composite_requirement_source_flattens_and_deduplicates_sources() -> Result<()> {
        let first_source = RequirementSource::SystemRequirementsToml {
            file: system_requirements_toml_file_for_test()?,
        };
        let second_source = RequirementSource::Unknown;

        assert_eq!(
            RequirementSource::composite([
                first_source.clone(),
                RequirementSource::composite([second_source.clone(), first_source.clone()]),
            ]),
            RequirementSource::Composite {
    ...
    fn merge_unset_fields_copies_every_field_and_sets_sources() {
        ...
        let source = system_requirements_source_for_test();
    ...
    fn merge_unset_fields_does_not_overwrite_existing_values() -> Result<()> {
        let existing_source = system_requirements_source_for_test();
...
"#,
    ...
    fn constrained_fields_store_requirement_source() -> Result<()> {
        ...
        let source_location = system_requirements_source_for_test();
        let mut target = ConfigRequirementsWithSources::default();
        target.merge_unset_fields(source_location.clone(), source);
        let requirements = ConfigRequirements::try_from(target)?;
    ...
    fn remote_sandbox_config_does_not_override_higher_precedence_sandbox_modes() -> Result<()> {
        let high_source = system_requirements_source_for_test();
...
command = "python3 /system/hooks/pre.py"
        ...
        assert_eq!(
            ...
            Some(std::path::Path::new("/managed/hooks"))
        );
        assert_eq!(
            target.hooks.as_ref().map(|hooks| hooks.source.clone()),
            Some(system_requirements_source_for_test())
        );
        Ok(())
    }
