...
#[cfg(unix)]
const SYSTEM_CONFIG_TOML_FILE_UNIX: &str = "/etc/sprite/config.toml";
...
// These settings are still supported from user, system, and runtime config
// layers.
...
///
/// - system    `/etc/sprite/requirements.toml` (Unix) or
///   `%ProgramData%\Sprite\requirements.toml` (Windows)
///
/// Configuration is built up from multiple layers in the following order:
///
/// - system    `/etc/sprite/config.toml` (Unix) or
///   `%ProgramData%\Sprite\config.toml` (Windows)
/// - user      `${SPRITE_HOME}/config.toml`
/// - profile   `${SPRITE_HOME}/<name>.config.toml`, when selected
/// - cwd       `${PWD}/config.toml` (loaded but disabled when the directory is untrusted)
/// - tree      parent directories up to root looking for `./.sprite/config.toml` (loaded but disabled when untrusted)
/// - repo      `$(git rev-parse --show-toplevel)/.sprite/config.toml` (loaded but disabled when untrusted)
/// - runtime   e.g., --config flags, model selector in UI
///
/// See the Sprite runtime migration docs for details.
///
...
pub async fn load_config_layers_state(
    ...
    // contain overrides.
    let active_user_file = overrides.user_config_path(sprite_home)?;
    let base_user_file = AbsolutePathBuf::resolve_path_against_base(CONFIG_TOML_FILE, sprite_home);
...
fn system_requirements_toml_file() -> io::Result<AbsolutePathBuf> {
    AbsolutePathBuf::from_absolute_path(Path::new("/etc/sprite/requirements.toml"))
}
...
#[cfg(windows)]
fn windows_sprite_system_dir() -> PathBuf {
...
impl ProjectTrustContext {
    ...
    fn root_checkout_hooks_folder_for_dir(&self, dir: &AbsolutePathBuf) -> Option<AbsolutePathBuf> {
        ...
        Some(repo_root.join(relative_dir).join(".sprite"))
    }
}
...
fn project_layer_entry(
    ...
    let source = ConfigLayerSource::Project {
        project_config_dir: dot_sprite_folder.clone(),
    };
...
async fn load_project_layers(
    ...
    let sprite_home_abs = AbsolutePathBuf::from_absolute_path(sprite_home)?;
    let sprite_home_normalized =
        normalize_path(sprite_home_abs.as_path()).unwrap_or_else(|_| sprite_home_abs.to_path_buf());
    let mut dirs = cwd
        .ancestors()
        .scan(false, |done, a| {
            if *done {
    ...
    for dir in dirs {
        let dot_sprite_abs = dir.join(".sprite");
        if !fs
            .get_metadata(&dot_sprite_abs, /*sandbox*/ None)
            .await
        ...
        let dot_sprite_normalized = normalize_path(dot_sprite_abs.as_path())
            .unwrap_or_else(|_| dot_sprite_abs.to_path_buf());
        if dot_sprite_abs == sprite_home_abs || dot_sprite_normalized == sprite_home_normalized {
            continue;
        }
        let config_file = dot_sprite_abs.join(CONFIG_TOML_FILE);
            ...
            Ok(contents) => {
                ...
                if disabled_reason.is_none() && strict_config {
                    validate_config_toml_strictly(
                        ...
                        &contents,
                        &config,
                        dot_sprite_abs.as_path(),
                    )?;
                }
                let ignored_project_config_keys = sanitize_project_config(&mut config);
                let config =
                    resolve_relative_paths_in_config_toml(config, dot_sprite_abs.as_path())?;
    ...
    fn windows_system_requirements_toml_file_uses_expected_suffix() {
        ...
        assert!(
            windows_system_requirements_toml_file()
                .expect("requirements.toml path")
                .as_path()
                .ends_with(Path::new("Sprite").join("requirements.toml"))
        );
    }
    ...
    fn windows_system_config_toml_file_uses_expected_suffix() {
        ...
        assert!(
            windows_system_config_toml_file()
                .expect("config.toml path")
                .as_path()
                .ends_with(Path::new("Sprite").join("config.toml"))
        );
    }
}
