//! Registry of model providers supported by Sprite.
//!
//! Providers can be defined in two places:
//!   1. Built-in defaults compiled into the binary so Sprite works out-of-the-box.
//!   2. User-defined entries inside the user config under the `model_providers`
...
pub enum ProviderConfigError {
    #[error("environment variable {var} is required for this provider")]
    EnvVar {
        var: String,
...
impl ModelProviderInfo {
    ...
    fn build_header_map(&self) -> ProviderResult<HeaderMap> {
    ...
    pub fn api_key(&self) -> ProviderResult<Option<String>> {
        match &self.env_key {
            Some(env_key) => {
                let api_key = std::env::var(env_key)
                    .ok()
                    .filter(|v| !v.trim().is_empty())
                    .ok_or_else(|| ProviderConfigError::EnvVar {
                        var: env_key.clone(),
...
pub fn built_in_model_providers(
    ...
    // Keep the phase-1 base provider-neutral. Users can add cloud providers,
    // including AWS Bedrock-compatible endpoints, through config.toml.
    [
...
pub fn create_oss_provider(default_provider_port: u16, wire_api: WireApi) -> ModelProviderInfo {
    ...
    let oss_base_url = std::env::var("SPRITE_OSS_BASE_URL")
        .ok()
        .filter(|v| !v.trim().is_empty())
