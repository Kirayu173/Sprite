...
/// See provider documentation for reasoning summaries
#[derive(
    Debug, Serialize, Deserialize, Default, Clone, Copy, PartialEq, Eq, Display, JsonSchema, TS,
)]
...
/// Validated plain profile-v2 name used to select `$SPRITE_HOME/<name>.config.toml`.
...
/// Collaboration mode for an agent session.
...
mod tests {
    ...
    fn web_search_tool_config_merge_prefers_overlay_values() {
        let base = WebSearchToolConfig {
            ...
            allowed_domains: Some(vec!["sprite.dev".to_string()]),
