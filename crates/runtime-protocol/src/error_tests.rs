...
fn server_overloaded_maps_to_protocol() {
    let err = RuntimeError::ServerOverloaded;
    assert_eq!(
        err.to_runtime_protocol_error(),
        RuntimeErrorInfo::ServerOverloaded
    );
}
...
fn sandbox_denied_uses_aggregated_output_when_stderr_empty() {
    ...
    let err = RuntimeError::Sandbox(SandboxErr::Denied {
...
fn to_error_event_handles_response_stream_failed() {
    ...
    assert_eq!(
        event.error_info,
        Some(RuntimeErrorInfo::ResponseStreamConnectionFailed {
...
#[test]
fn usage_limit_reached_error_hides_upsell_for_non_default_limit_name() {
    let base = Utc.with_ymd_and_hms(2024, 1, 1, 0, 0, 0).unwrap();
    let resets_at = base + ChronoDuration::hours(1);
    with_now_override(base, move || {
        let expected_time = format_retry_timestamp(&resets_at);
...
fn unexpected_status_prefers_error_message_when_present() {
    let err = UnexpectedResponseError {
        ...
        body: r#"{"error":{"message":"Workspace is not authorized in this region."},"status":401}"#
            .to_string(),
        url: Some("https://api.sprite.dev/backend/responses".to_string()),
        cf_ray: None,
        request_id: Some("req-123".to_string()),
        identity_authorization_error: None,
        identity_error_code: None,
    ...
    assert_eq!(
        err.to_string(),
        format!(
            "unexpected status {status}: Workspace is not authorized in this region., url: https://api.sprite.dev/backend/responses, request id: req-123"
        )
    );
}
...
fn unexpected_status_includes_cf_ray_and_request_id() {
    ...
    assert_eq!(
        err.to_string(),
        format!(
            "unexpected status {status}: plain text error, url: https://api.sprite.dev/backend/responses, cf-ray: 9c81f9f18f2fa49d-LHR, request id: req-xyz"
        )
    );
}
...
fn unexpected_status_includes_identity_auth_details() {
    let err = UnexpectedResponseError {
        status: StatusCode::UNAUTHORIZED,
        body: "plain text error".to_string(),
        url: Some("https://api.sprite.dev/backend/models".to_string()),
    ...
    assert_eq!(
        err.to_string(),
        format!(
            "unexpected status {status}: plain text error, url: https://api.sprite.dev/backend/models, cf-ray: cf-ray-auth-401-test, request id: req-auth, auth error: missing_authorization_header, auth error code: token_expired"
        )
    );
}
