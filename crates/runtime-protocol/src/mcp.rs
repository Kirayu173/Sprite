...
//! Sprite runtime protocol.
//!
//! We intentionally keep these types TS/JSON-schema friendly (via `ts-rs` and
//! `schemars`) so they can be embedded in the runtime's own protocol structures.
...
// === Adapter helpers ===
//
// These types and conversions intentionally live in `runtime-protocol` so other crates can convert
