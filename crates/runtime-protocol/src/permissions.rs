...
const PROTECTED_METADATA_SPRITE_PATH_NAME: &str = ".sprite";
...
pub const PROTECTED_METADATA_PATH_NAMES: &[&str] = &[
    ...
    PROTECTED_METADATA_SPRITE_PATH_NAME,
];
...
pub fn is_protected_metadata_directory_name(name: &OsStr) -> bool {
    name == OsStr::new(PROTECTED_METADATA_AGENTS_PATH_NAME)
        || name == OsStr::new(PROTECTED_METADATA_SPRITE_PATH_NAME)
}
...
pub enum FileSystemSpecialPath {
    ...
    /// representable so config from a newer runtime degrades to warn-and-ignore
    /// instead of failing to load. Runtime 0.112.0 rejected unknown values here,
...
const PROJECT_ROOTS_GLOB_PATTERN_PREFIX: &str = "sprite-project-roots://";
...
impl FileSystemSandboxPolicy {
    ...
    pub fn workspace_write(
        ...
        append_default_read_only_project_root_subpath_if_no_explicit_rule(&mut entries, ".sprite");
        for writable_root in writable_roots {
            for protected_path in default_read_only_subpaths_for_writable_root(
                writable_root,
                /*protect_missing_dot_sprite*/ false,
            ) {
    ...
    pub fn from_legacy_sandbox_policy_for_cwd(sandbox_policy: &SandboxPolicy, cwd: &Path) -> Self {
        ...
        if let SandboxPolicy::WorkspaceWrite { writable_roots, .. } = sandbox_policy {
            if let Ok(cwd_root) = AbsolutePathBuf::from_absolute_path(cwd) {
                for protected_path in default_read_only_subpaths_for_writable_root(
                    &cwd_root, /*protect_missing_dot_sprite*/ true,
                ) {
    ...
    pub fn with_additional_legacy_workspace_writable_roots(
        ...
        for path in additional_writable_roots {
            ...
            for protected_path in default_read_only_subpaths_for_writable_root(
                path, /*protect_missing_dot_sprite*/ false,
            ) {
                append_default_read_only_path_if_no_explicit_rule(
                    &mut self.entries,
                    protected_path,
    ...
    pub fn get_writable_roots_with_cwd(&self, cwd: &Path) -> Vec<WritableRoot> {
        ...
        .map(|root| {
            ...
            let protect_missing_dot_sprite = AbsolutePathBuf::from_absolute_path(cwd)
                .ok()
                .is_some_and(|cwd| normalize_effective_absolute_path(cwd) == root);
            let mut read_only_subpaths: Vec<AbsolutePathBuf> =
                default_read_only_subpaths_for_writable_root(&root, protect_missing_dot_sprite)
                    .into_iter()
            ...
            // Example: if `<root>/.sprite -> <root>/decoy`, bwrap must still see
            // `<root>/.sprite`, not only the resolved `<root>/decoy`.
            ...
            WritableRoot {
                ...
                // `.sprite` so downstream sandboxes can still detect and mask
...
pub(crate) fn default_read_only_subpaths_for_writable_root(
    ...
    protect_missing_dot_sprite: bool,
) -> Vec<AbsolutePathBuf> {
    ...
    // Keep top-level project metadata under .sprite read-only to the agent by
    ...
    let top_level_sprite = writable_root.join(PROTECTED_METADATA_SPRITE_PATH_NAME);
    if protect_missing_dot_sprite || top_level_sprite.as_path().is_dir() {
        subpaths.push(top_level_sprite);
    }
...
mod tests {
    ...
    #[cfg(unix)]
    const SYMLINKED_TMPDIR_TEST_ENV: &str = "SPRITE_PROTOCOL_TEST_SYMLINKED_TMPDIR";
    ...
    fn legacy_workspace_write_projection_preserves_symbolic_project_root() {
        ...
        assert_eq!(
            ...
            FileSystemSandboxPolicy::restricted(vec![
                ...
                FileSystemSandboxEntry {
                    path: FileSystemPath::Special {
                        value: FileSystemSpecialPath::project_roots(Some(".sprite".into())),
                    },
                    access: FileSystemAccessMode::Read,
                },
            ])
    ...
    #[cfg(unix)]
    #[test]
    fn writable_roots_skip_default_dot_sprite_when_explicit_user_rule_exists() {
        let cwd = TempDir::new().expect("tempdir");
        let expected_root = AbsolutePathBuf::from_absolute_path(
            cwd.path().canonicalize().expect("canonicalize cwd"),
        )
        .expect("absolute canonical root");
        let explicit_dot_sprite = expected_root.join(".sprite");
        ...
        let policy = FileSystemSandboxPolicy::restricted(vec![
            ...
            FileSystemSandboxEntry {
                path: FileSystemPath::Path {
                    path: explicit_dot_sprite.clone(),
                },
                access: FileSystemAccessMode::Write,
            },
        ]);
        ...
        assert!(
            !workspace_root
                .protected_metadata_names
                .contains(&".sprite".to_string()),
            "explicit .sprite rule should remove the metadata-name protection"
        );
        assert!(
            !workspace_root
                .read_only_subpaths
    ...
    fn filesystem_policy_blocks_protected_metadata_path_writes_by_default() {
        ...
        let dot_sprite_config = cwd.path().join(".sprite").join("config.toml");
        ...
        assert!(!file_system_policy.can_write_path_with_cwd(&dot_sprite_config, cwd.path()));
        ...
        assert!(!writable_roots[0].is_path_writable(&dot_sprite_config));
    }
    ...
    fn legacy_workspace_write_projection_accepts_relative_cwd() {
        ...
        expected_entries.extend(
            default_read_only_subpaths_for_writable_root(
                &expected_root,
                /*protect_missing_dot_sprite*/ true,
            )
            .into_iter()
            .map(|path| FileSystemSandboxEntry {
                path: FileSystemPath::Path { path },
        ...
        assert!(
            !file_system_policy
                .can_write_path_with_cwd(Path::new(".sprite/config.toml"), relative_cwd,)
        );
        assert!(
    ...
    fn effective_runtime_roots_preserve_symlinked_paths() {
        ...
        let sprite_dir = real_root.join(".sprite");

        fs::create_dir_all(&blocked).expect("create blocked");
        fs::create_dir_all(&sprite_dir).expect("create .sprite");
        ...
        let expected_sprite = link_root.join(".sprite");
    ...
    fn project_roots_special_path_preserves_symlinked_root() {
        ...
        let expected_sprite = expected_root.join(".sprite");
    ...
    fn writable_roots_preserve_symlinked_protected_subpaths() {
        ...
        let decoy = root.join("decoy-sprite");
        let dot_sprite = root.join(".sprite");
        fs::create_dir_all(&decoy).expect("create decoy");
        symlink_dir(&decoy, &dot_sprite).expect("create .sprite symlink");

        let root = AbsolutePathBuf::from_absolute_path(&root).expect("absolute root");
        let expected_dot_sprite = AbsolutePathBuf::from_absolute_path(
            root.as_path()
                .canonicalize()
                .expect("canonicalize root")
                .join(".sprite"),
        )
        .expect("absolute .sprite symlink");
    ...
    fn tmpdir_special_path_preserves_symlinked_tmpdir() {
        ...
        let sprite_dir = real_tmpdir.join(".sprite");
