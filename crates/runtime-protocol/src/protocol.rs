//! Defines the protocol for a runtime session between a client and an agent.
//!
...
pub enum Op {
    ...
    /// Request the runtime to drop the last N user turns from in-memory context.
    ///
...
/// privileges of the agent (e.g. `.sprite`, `.git`, notably `.git/hooks`) under
...
pub struct ErrorEvent {
    ...
    pub error_info: Option<RuntimeErrorInfo>,
}
...
impl ErrorEvent {
    ...
    pub fn affects_turn_status(&self) -> bool {
        self.error_info
            .as_ref()
            .is_none_or(RuntimeErrorInfo::affects_turn_status)
    }
}
...
impl SessionSource {
    ...
    pub fn restriction_platform(&self) -> Option<AppPlatform> {
        match self {
            SessionSource::Custom(source) => AppPlatform::from_session_source_name(source),
            SessionSource::Cli
            | SessionSource::VSCode
            | SessionSource::Exec
            | SessionSource::Mcp
            | SessionSource::Unknown => Some(AppPlatform::Agent),
            SessionSource::Internal(_) | SessionSource::SubAgent(_) => None,
        }
    }

    pub fn matches_platform_restriction(&self, platforms: &[AppPlatform]) -> bool {
        platforms.is_empty()
            || self
                .restriction_platform()
                .is_some_and(|platform| platform.matches_platform_restriction(platforms))
    }
...
impl From<crate::model_capabilities::TruncationPolicyConfig> for TruncationPolicy {
    ...
    pub fn token_budget(&self) -> usize {
        match self {
            TruncationPolicy::Bytes(bytes) => {
                usize::try_from(utils_string::approx_tokens_from_byte_count(*bytes))
                    .unwrap_or(usize::MAX)
            }
...
impl AppPlatform {
    ...
    pub fn matches_platform_restriction(&self, platforms: &[AppPlatform]) -> bool {
        platforms.is_empty() || platforms.contains(self)
    }
}
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize, JsonSchema, TS)]
#[serde(rename_all = "snake_case")]
...
mod tests {
    ...
    #[test]
    fn session_source_restriction_platform_defaults_non_subagent_sources_to_agent() {
        assert_eq!(
            SessionSource::Cli.restriction_platform(),
            Some(AppPlatform::Agent)
        );
        assert_eq!(
            SessionSource::VSCode.restriction_platform(),
            Some(AppPlatform::Agent)
        );
        assert_eq!(
            SessionSource::Exec.restriction_platform(),
            Some(AppPlatform::Agent)
        );
        assert_eq!(
            SessionSource::Mcp.restriction_platform(),
            Some(AppPlatform::Agent)
        );
        assert_eq!(
            SessionSource::Unknown.restriction_platform(),
            Some(AppPlatform::Agent)
        );
    }

    #[test]
    fn session_source_restriction_platform_does_not_guess_subagent_platforms() {
        assert_eq!(
            SessionSource::SubAgent(SubAgentSource::Review).restriction_platform(),
            None
        );
        assert_eq!(
    ...
    #[test]
    fn session_source_restriction_platform_maps_custom_sources_to_platforms() {
        assert_eq!(
            SessionSource::Custom("chat".to_string()).restriction_platform(),
            Some(AppPlatform::Chat)
        );
        assert_eq!(
            SessionSource::Custom("WORKSPACE".to_string()).restriction_platform(),
            Some(AppPlatform::Workspace)
        );
        assert_eq!(
            SessionSource::Custom("agent".to_string()).restriction_platform(),
            Some(AppPlatform::Agent)
        );
        assert_eq!(
            SessionSource::Custom("workspace-dev".to_string()).restriction_platform(),
            None
        );
    }

    #[test]
    fn session_source_matches_platform_restriction() {
        assert!(
            SessionSource::Custom("chat".to_string())
                .matches_platform_restriction(&[AppPlatform::Chat])
        );
        assert!(
            !SessionSource::Custom("chat".to_string())
                .matches_platform_restriction(&[AppPlatform::Agent])
        );
        assert!(SessionSource::VSCode.matches_platform_restriction(&[AppPlatform::Agent]));
        assert!(
            !SessionSource::Custom("workspace-dev".to_string())
                .matches_platform_restriction(&[AppPlatform::Workspace])
        );
        assert!(
            SessionSource::Custom("workspace-dev".to_string()).matches_platform_restriction(&[])
        );
    }
    ...
    fn restricted_file_system_policy_treats_root_with_carveouts_as_scoped_access() {
        ...
        let canonical_cwd = utils_absolute_path::canonicalize_preserving_symlinks(cwd.path())
        ...
        let expected_blocked = AbsolutePathBuf::from_absolute_path(
            utils_absolute_path::canonicalize_preserving_symlinks(cwd.path())
                .expect("canonicalize cwd")
                .join("blocked"),
        )
    ...
    fn restricted_file_system_policy_derives_effective_paths() {
        ...
        std::fs::create_dir_all(cwd.path().join(".sprite")).expect("create .sprite");
        let canonical_cwd = utils_absolute_path::canonicalize_preserving_symlinks(cwd.path())
            .expect("canonicalize cwd");
        let cwd_absolute =
        ...
        let expected_sprite = AbsolutePathBuf::from_absolute_path(canonical_cwd.join(".sprite"))
        ...
        assert!(
            writable_roots[0]
                .read_only_subpaths
                .iter()
                .any(|path| path.as_path() == expected_sprite.as_path())
        );
    }
        ...
        assert_eq!(
            ...
            vec![
                (
                    canonical_cwd,
                    vec![
                        expected_dot_sprite.to_path_buf(),
    ...
    fn active_turn_not_steerable_error_does_not_affect_turn_status() {
        let event = ErrorEvent {
            ...
            error_info: Some(RuntimeErrorInfo::ActiveTurnNotSteerable {
    ...
    fn generic_error_affects_turn_status() {
        let event = ErrorEvent {
            ...
            error_info: Some(RuntimeErrorInfo::Other),
        };
        assert!(event.affects_turn_status());
    }
    ...
    fn serialize_event() -> Result<()> {
        ...
        let event = Event {
            ...
            msg: EventMsg::SessionConfigured(SessionConfiguredEvent {
                ...
                thread_source: None,
                thread_name: None,
                model: "sprite-mini-latest".to_string(),
                model_provider_id: "sprite".to_string(),
                service_tier: None,
                approval_policy: AskForApproval::Never,
        ...
        let expected = json!({
            ...
            "msg": {
                ...
                "model": "sprite-mini-latest",
                "model_provider_id": "sprite",
                "approval_policy": "never",
