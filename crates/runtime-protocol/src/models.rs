...
pub enum SandboxEnforcement {
    /// The runtime owns sandbox construction for this profile.
    #[default]
    Managed,
...
/// Filesystem permissions for profiles where the runtime owns sandbox construction.
...
fn local_image_error_placeholder(
    ...
    ContentItem::InputText {
        text: format!(
            "Sprite could not read the local image at `{}`: {}",
            path.display(),
            error
        ),
    }
...
fn unsupported_image_error_placeholder(path: &std::path::Path, mime: &str) -> ContentItem {
    ContentItem::InputText {
        text: format!(
            "Sprite cannot attach image at `{}`: unsupported image `{}`.",
            path.display(),
            mime
        ),
    }
...
fn convert_mcp_content_to_items(
    ...
    const IMAGE_DETAIL_META_KEY: &str = "sprite/imageDetail";
            ...
            Ok(McpContent::Image {
                ...
                FunctionCallOutputContentItem::InputImage {
                    ...
                    detail: meta
                        ...
                        .and_then(|meta| meta.get(IMAGE_DETAIL_META_KEY))
...
// (Moved event mapping logic into runtime-core to avoid coupling protocol to UI-facing events.)
...
mod tests {
    ...
    fn function_call_deserializes_optional_namespace() {
        let item: ResponseItem = serde_json::from_value(serde_json::json!({
            "type": "function_call",
            "name": "mcp__sprite_apps__gmail_get_recent_emails",
            "namespace": "mcp__sprite_apps__gmail",
            "arguments": "{\"top_k\":5}",
            "call_id": "call-1",
        }))
        ...
        assert_eq!(
            ...
            ResponseItem::FunctionCall {
                id: None,
                name: "mcp__sprite_apps__gmail_get_recent_emails".to_string(),
                namespace: Some("mcp__sprite_apps__gmail".to_string()),
    ...
    fn tool_search_output_roundtrips() -> Result<()> {
        let input = ResponseInputItem::ToolSearchOutput {
            ...
            tools: vec![serde_json::json!({
                "type": "function",
                "name": "mcp__sprite_apps__calendar_create_event",
...
<svg xmlns="http://www.w3.org/2000/svg" width="1" height="1"></svg>"#,
        ...
        match item {
            ResponseInputItem::Message { content, .. } => {
                assert_eq!(content.len(), 1);
                let expected = format!(
                    "Sprite cannot attach image at `{}`: unsupported image `image/svg+xml`.",
                    svg_path.display()
                );
                match &content[0] {
