...
//! - sensitive subpaths such as `.git`, `.agents`, and `.sprite` remain
...
use runtime_protocol::permissions::is_protected_metadata_name;
use runtime_protocol::protocol::FileSystemAccessMode;
use runtime_protocol::protocol::FileSystemPath;
use runtime_protocol::protocol::FileSystemSandboxPolicy;
use runtime_protocol::protocol::FileSystemSpecialPath;
use runtime_protocol::protocol::WritableRoot;
    ...
    let missing_auto_metadata_read_only_project_root_subpaths: HashSet<PathBuf> =
        file_system_sandbox_policy
            ...
            .filter_map(|entry| {
                ...
                // protection consistently for `.git`, `.agents`, and `.sprite`.
                ...
                if project_subpath != Path::new(".git")
                    ...
                    && project_subpath != Path::new(".sprite")
                {
                    return None;
                }
...
mod tests {
    ...
    use runtime_protocol::protocol::FileSystemAccessMode;
    use runtime_protocol::protocol::FileSystemPath;
    use runtime_protocol::protocol::FileSystemSandboxEntry;
    use runtime_protocol::protocol::FileSystemSandboxPolicy;
    use runtime_protocol::protocol::FileSystemSpecialPath;
    ...
    fn writable_roots_under_symlinked_ancestors_bind_real_target() {
        ...
        let logical_codex = logical_home.join(".sprite");
        ...
        std::os::unix::fs::symlink(&real_codex, &logical_codex)
            .expect("create symlinked sprite home");
    ...
    fn missing_read_only_subpath_uses_empty_file_bind_data() {
        ...
        assert_empty_directory_mounted_read_only(&args.args, &workspace.join(".sprite"));
    ...
    fn missing_project_root_metadata_carveouts_use_metadata_path_masks() {
        ...
        let dot_codex = path_to_string(&temp_dir.path().join(".sprite"));
