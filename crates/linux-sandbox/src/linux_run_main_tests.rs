...
#[cfg(test)]
use runtime_protocol::protocol::NetworkSandboxPolicy;
#[cfg(test)]
...
fn inserts_bwrap_argv0_before_command_separator() {
    ...
    apply_inner_command_argv0_for_launcher(
        &mut argv,
        /*supports_argv0*/ true,
        "/tmp/sprite-arg0-session/sprite-linux-sandbox".to_string(),
    );
    assert_eq!(
        argv,
        vec![
            ...
            "/proc".to_string(),
            "--argv0".to_string(),
            "sprite-linux-sandbox".to_string(),
            "--".to_string(),
            "/bin/true".to_string(),
        ]
    );
...
fn rewrites_inner_command_path_when_bwrap_lacks_argv0() {
    ...
    assert!(
        argv.windows(2)
            .any(|window| { window == ["--", "/tmp/sprite-arg0-session/sprite-linux-sandbox"] })
    );
}
...
fn split_only_filesystem_policy_requires_direct_runtime_enforcement() {
    ...
    let policy = FileSystemSandboxPolicy::restricted(vec![
        runtime_protocol::permissions::FileSystemSandboxEntry {
            path: runtime_protocol::permissions::FileSystemPath::Special {
                value: runtime_protocol::permissions::FileSystemSpecialPath::project_roots(
                    /*subpath*/ None,
                ),
            },
            access: runtime_protocol::permissions::FileSystemAccessMode::Write,
        },
        runtime_protocol::permissions::FileSystemSandboxEntry {
            path: runtime_protocol::permissions::FileSystemPath::Path { path: docs },
            access: runtime_protocol::permissions::FileSystemAccessMode::Read,
        },
    ]);
...
fn root_write_read_only_carveout_requires_direct_runtime_enforcement() {
    ...
    let policy = FileSystemSandboxPolicy::restricted(vec![
        runtime_protocol::permissions::FileSystemSandboxEntry {
            path: runtime_protocol::permissions::FileSystemPath::Special {
                value: runtime_protocol::permissions::FileSystemSpecialPath::Root,
            },
            access: runtime_protocol::permissions::FileSystemAccessMode::Write,
        },
        runtime_protocol::permissions::FileSystemSandboxEntry {
...
fn synthetic_mount_registry_root_is_unique_to_effective_user() {
    ...
    assert_eq!(
        ...
        std::env::temp_dir().join(format!(
            "sprite-bwrap-synthetic-mount-targets-{effective_uid}"
        ))
    );
}
