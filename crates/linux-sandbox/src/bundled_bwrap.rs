...
use install_context::InstallContext;
...
fn legacy_candidates_for_exe(exe: &Path) -> Vec<PathBuf> {
    ...
    candidates.push(exe_dir.join("sprite-resources").join("bwrap"));
    if let Some(package_target_dir) = exe_dir.parent() {
        candidates.push(package_target_dir.join("sprite-resources").join("bwrap"));
    }
...
fn expected_sha256() -> Option<[u8; 32]> {
    ...
    *EXPECTED.get_or_init(|| {
        let raw_digest = option_env!("SPRITE_BWRAP_SHA256")?;
        let digest = parse_sha256_hex(raw_digest)
            .unwrap_or_else(|err| panic!("invalid SPRITE_BWRAP_SHA256 value: {err}"));
...
mod tests {
    use super::*;
    use install_context::InstallContext;
    use install_context::InstallMethod;
    use install_context::SpritePackageLayout;
    ...
    fn finds_package_layout_bwrap_from_install_context() {
        ...
        let resources_dir = package_dir.join("sprite-resources");
    ...
    fn finds_legacy_standalone_bundled_bwrap_next_to_exe_resources() {
        ...
        let expected_bwrap = temp_dir.path().join("sprite-resources").join("bwrap");
    ...
    fn finds_npm_bundled_bwrap_next_to_target_vendor_dir() {
        ...
        let expected_bwrap = target_dir.join("sprite-resources").join("bwrap");
        write_executable(&exe);
        write_executable(&expected_bwrap);
