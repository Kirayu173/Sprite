...
use runtime_core::exec::ExecCapturePolicy;
use runtime_core::exec::ExecParams;
use runtime_core::exec::process_exec_tool_call;
use runtime_core::exec_env::create_env;
use runtime_core::sandboxing::SandboxPermissions;
use runtime_protocol::config_types::ShellEnvironmentPolicy;
use runtime_protocol::config_types::WindowsSandboxLevel;
use runtime_protocol::error::CodexErr;
use runtime_protocol::error::Result;
use runtime_protocol::error::SandboxErr;
...
fn sprite_linux_sandbox_exe() -> PathBuf {
    let sandbox_program = PathBuf::from(env!("CARGO_BIN_EXE_sprite-linux-sandbox"));
...
async fn run_cmd_output(
    ...
    timeout_ms: u64,
) -> runtime_protocol::exec_output::ExecToolCallOutput {
...
async fn run_cmd_result_with_writable_roots(
    ...
    network_access: bool,
) -> Result<runtime_protocol::exec_output::ExecToolCallOutput> {
...
async fn run_cmd_result_with_permission_profile_for_cwd(
    ...
    let sprite_linux_sandbox_exe = Some(sprite_linux_sandbox_exe());
...
fn is_bwrap_unavailable_output(output: &runtime_protocol::exec_output::ExecToolCallOutput) -> bool {
...
fn expect_denied(
    result: Result<runtime_protocol::exec_output::ExecToolCallOutput>,
    context: &str,
) -> runtime_protocol::exec_output::ExecToolCallOutput {
    match result {
        Ok(output) => {
...
async fn assert_network_blocked(cmd: &[&str]) {
    ...
    let sprite_linux_sandbox_exe: Option<PathBuf> = Some(sprite_linux_sandbox_exe());
...
async fn sandbox_blocks_git_and_codex_writes_inside_writable_root() {
    ...
    let dot_codex = tmpdir.path().join(".sprite");
    std::fs::create_dir_all(&dot_git).expect("create .git");
    std::fs::create_dir_all(&dot_codex).expect("create .sprite");
    ...
    let codex_output = expect_denied(
        ...
        .await,
        ".sprite write should be denied under bubblewrap",
    );
...
async fn sandbox_blocks_codex_symlink_replacement_attack() {
    ...
    let dot_codex = tmpdir.path().join(".sprite");
    symlink(&decoy, &dot_codex).expect("create .sprite symlink");
    ...
    let codex_output = expect_denied(
        ...
        .await,
        ".sprite symlink replacement should be denied",
    );
    assert_ne!(codex_output.exit_code, 0);
}
...
async fn sandbox_reports_codex_symlink_build_failure_without_panicking() {
    ...
    {
        Err(CodexErr::Sandbox(SandboxErr::Denied { output, .. })) => *output,
        result => panic!(".sprite symlink build failure should deny: {result:?}"),
    };
...
"#,
    ...
    let mkdir_codex_output = expect_denied(
        ...
        .await,
        "child .sprite directory creation should be denied",
    );
    assert_ne!(mkdir_codex_output.exit_code, 0);
    assert!(!subdir.join(".sprite").exists());
...
async fn sandbox_blocks_explicit_split_policy_carveouts_under_bwrap() {
    ...
    let sandbox_helper_dir = sprite_linux_sandbox_exe()
        .parent()
