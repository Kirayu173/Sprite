# sprite-linux-sandbox

This crate is responsible for producing:

- a `sprite-linux-sandbox` standalone executable for Linux that is bundled with the Node.js version of the Sprite CLI
- a lib crate that exposes the business logic of the executable as `run_main()` so that
  - the `sprite-exec` CLI can check if its arg0 is `sprite-linux-sandbox` and, if so, execute as if it were `sprite-linux-sandbox`
  - this should also be true of the `codex` multitool CLI

On Linux, Sprite prefers the first `bwrap` found on `PATH`
...
the helper falls back to the bundled `sprite-resources/bwrap` binary shipped
with Sprite.
Sprite also surfaces a startup warning when `bwrap` is missing so users know it
is falling back to the bundled helper. Sprite surfaces the same startup warning
...
it cannot create the required user namespaces, so Sprite rejects sandboxed shell
...
- If `bwrap` is missing, the helper falls back to the bundled
  `sprite-resources/bwrap` path.
- If `bwrap` is missing, Sprite also surfaces a startup warning instead of
  printing directly from the sandbox helper.
- If bubblewrap cannot create user namespaces, Sprite surfaces a startup warning
  instead of waiting for a runtime sandbox failure.
- WSL2 uses the normal Linux bubblewrap path.
- WSL1 is not supported for bubblewrap sandboxing; Sprite rejects sandboxed
...
- When bubblewrap is active, protected subpaths under writable roots (for
  example `.git`,
  resolved `gitdir:`, and `.sprite`) are re-applied as read-only via `--ro-bind`.
