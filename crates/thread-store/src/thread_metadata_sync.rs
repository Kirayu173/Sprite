...
use runtime_protocol::protocol::USER_MESSAGE_BEGIN;
        ...
        for item in items {
            match item {
                ...
                RolloutItem::EventMsg(EventMsg::TokenCount(token_count)) => {
                    ...
                    if let Some(usage_limits) = token_count.usage_limits.as_ref() {
                        update.usage_limits = Some(usage_limits.clone());
                    }
                }
...
mod tests {
    ...
    use runtime_protocol::protocol::ThreadGoal;
    use runtime_protocol::protocol::ThreadGoalStatus;
    use runtime_protocol::protocol::ThreadGoalUpdatedEvent;
    use runtime_protocol::protocol::TokenCountEvent;
    ...
    #[test]
    fn token_count_observes_usage_limit_snapshot() {
        let thread_id = ThreadId::new();
        let snapshot = UsageLimitSnapshot {
            ...
            primary: Some(UsageLimitWindow {
                ...
                resets_at: Some(1_700_020_000),
            }),
            secondary: None,
            kind: Some(UsageLimitKind::ProjectLimitReached),
        };
        let sync = ThreadMetadataSync::for_resume(&resume_params(
            thread_id,
        ...
        let update = sync.take_pending_update().expect("pending metadata update");
        assert_eq!(update.patch.usage_limits, Some(snapshot));
    }
