...
use runtime_protocol::protocol::NetworkAccess;
...
pub(super) fn rollout_path_is_archived(sprite_home: &Path, path: &Path) -> bool {
    path.starts_with(sprite_home.join(ARCHIVED_SESSIONS_SUBDIR))
        || path
            .components()
...
pub(super) fn stored_thread_from_rollout_item(
    ...
    let preview = item
        ...
        .unwrap_or_default();
    let rollout_path = rollout::plain_rollout_path(item.path.as_path());
