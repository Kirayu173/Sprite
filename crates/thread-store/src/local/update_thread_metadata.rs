...
async fn refresh_resolved_rollout_path(resolved: &mut ResolvedRolloutPath) {
    if let Some(path) = rollout::existing_rollout_path(resolved.path.as_path()).await {
        resolved.path = path;
    }
}
...
async fn apply_metadata_update(
    ...
    let sqlite_write_result: ThreadStoreResult<()> = if let Some(state_db) = state_db.as_ref() {
        ...
        async {
            ...
            if let Some(usage_limits) = patch.usage_limits {
                metadata.usage_limits = serde_json::to_string(&usage_limits).ok();
            }
...
fn normalize_cwd(cwd: PathBuf) -> PathBuf {
    utils_path::normalize_for_path_comparison(cwd.as_path()).unwrap_or(cwd)
}
...
async fn apply_thread_name(
    ...
    append_thread_name(store.config.sprite_home.as_path(), thread_id, &name)
        .await
        .map_err(|err| ThreadStoreError::Internal {
...
fn rollout_path_is_archived(store: &LocalThreadStore, path: &Path) -> bool {
    path.starts_with(store.config.sprite_home.join(ARCHIVED_SESSIONS_SUBDIR))
}
...
mod tests {
    ...
    async fn update_thread_metadata_sets_git_info() {
            ...
            .update_thread_metadata(UpdateThreadMetadataParams {
                ...
                patch: ThreadMetadataPatch {
                    git_info: Some(GitInfoPatch {
                        ...
                        origin_url: Some(Some("https://github.com/openai/sprite".to_string())),
                    }),
                    ..Default::default()
                },
                include_archived: false,
        ...
        assert_eq!(
            ...
            Some("https://github.com/openai/sprite")
        );
    }
    ...
    async fn observed_metadata_normalizes_cwd_for_list_filters() {
        ...
        let normalized_cwd =
            utils_path::normalize_for_path_comparison(workspace.as_path()).expect("normalize cwd");
