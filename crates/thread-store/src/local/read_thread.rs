        ...
        && (params.include_archived
            || (metadata.archived_at.is_none()
                && !rollout_path_is_archived(
                    store.config.sprite_home.as_path(),
...
async fn sqlite_rollout_path_can_load_history_for_thread(
    store: &LocalThreadStore,
    path: &std::path::Path,
    thread_id: runtime_protocol::ThreadId,
) -> bool {
    if rollout::existing_rollout_path(path).await.is_none() {
        return false;
    }
...
async fn resolve_requested_rollout_path(
    ...
    let path = if rollout_path.is_relative() {
        store.config.sprite_home.join(rollout_path)
    } else {
        rollout_path
    };
    ...
    let Some(path) = rollout::existing_rollout_path(path.as_path()).await else {
        return Err(ThreadStoreError::InvalidRequest {
            message: format!(
...
async fn resolve_rollout_path(
    ...
    if let Ok(path) = live_writer::rollout_path(store, thread_id).await
        && rollout::existing_rollout_path(path.as_path())
            .await
            .is_some()
        && (include_archived
            || !rollout_path_is_archived(store.config.sprite_home.as_path(), &path))
    {
        return Ok(Some(path));
    }
...
async fn read_thread_from_rollout_path(
    ...
    let archived = rollout_path_is_archived(store.config.sprite_home.as_path(), path.as_path());
    ...
    thread.rollout_path = Some(rollout::plain_rollout_path(path.as_path()));
    ...
    if let Ok(Some(title)) =
        find_thread_name_by_id(store.config.sprite_home.as_path(), &thread.thread_id).await
    {
...
async fn load_history_items(
    path: &std::path::Path,
) -> ThreadStoreResult<Vec<runtime_protocol::protocol::RolloutItem>> {
...
async fn stored_thread_from_sqlite_metadata(
    ...
    let name = match distinct_thread_metadata_title(&metadata) {
        ...
        None => find_thread_name_by_id(store.config.sprite_home.as_path(), &metadata.id)
            .await
            .ok()
            .flatten()
    ...
    let rollout_path = rollout::plain_rollout_path(metadata.rollout_path.as_path());
    ...
    StoredThread {
        ...
        permission_profile,
        token_usage: Some(runtime_protocol::protocol::TokenUsage {
            total_tokens: metadata.tokens_used,
            ..Default::default()
        }),
        usage_limits: metadata
            .usage_limits
            .as_deref()
            .and_then(parse_usage_limits),
...
fn stored_thread_from_meta_line(
    ...
    let rollout_path = rollout::plain_rollout_path(path.as_path());
    StoredThread {
...
fn parse_usage_limits(value: &str) -> Option<runtime_protocol::protocol::UsageLimitSnapshot> {
...
mod tests {
    ...
    async fn read_thread_returns_rollout_path_summary() {
        ...
        let relative_path = active_path
            ...
            .expect("path should be under sprite home")
            .to_path_buf();
