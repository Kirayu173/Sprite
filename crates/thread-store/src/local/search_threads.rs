...
use rollout::first_rollout_content_match_snippet;
use rollout::parse_cursor;
use rollout::search_rollout_matches;
...
pub(super) async fn search_threads(
    ...
    let rollout_config = RolloutConfig {
        sprite_home: store.config.sprite_home.clone(),
        sqlite_home: store.config.sqlite_home.clone(),
        cwd: store.config.sprite_home.clone(),
...
async fn set_thread_search_result_names(
    ...
    if names.len() < thread_ids.len()
        && let Ok(legacy_names) =
            find_thread_names_by_ids(store.config.sprite_home.as_path(), &thread_ids).await
    {
