...
impl LocalThreadStoreConfig {
    pub fn from_config(config: &impl rollout::RolloutConfigView) -> Self {
        Self {
            sprite_home: config.sprite_home().to_path_buf(),
...
impl ThreadStore for LocalThreadStore {
        ...
        if let Ok(rollout_path) = live_writer::rollout_path(self, params.thread_id).await {
            if !params.include_archived
                && helpers::rollout_path_is_archived(
                    self.config.sprite_home.as_path(),
    ...
    async fn list_turns(&self, params: ListTurnsParams) -> ThreadStoreResult<TurnPage> {
        list_turn_items::list_turns(self, params).await
    }

    async fn list_items(&self, params: ListItemsParams) -> ThreadStoreResult<ItemPage> {
        list_turn_items::list_items(self, params).await
    }
