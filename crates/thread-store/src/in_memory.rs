...
use runtime_protocol::protocol::ThreadMemoryMode;
...
mod tests {
    ...
    #[tokio::test]
    async fn archive_and_unarchive_update_thread_state() {
        let store = InMemoryThreadStore::default();
        let thread_id = create_test_thread(&store).await;
    ...
    #[tokio::test]
    async fn read_thread_rejects_archived_thread_when_not_included() {
        let store = InMemoryThreadStore::default();
        let thread_id = create_test_thread(&store).await;
        store
            .archive_thread(ArchiveThreadParams { thread_id })
        ...
        let err = store
            ...
            .await
            .expect_err("active-only read should reject archived thread");
    ...
    #[tokio::test]
    async fn list_threads_selects_active_or_archived_threads() {
        let store = InMemoryThreadStore::default();
        let active_thread_id = create_test_thread(&store).await;
        let archived_thread_id = create_test_thread(&store).await;
        store
        ...
        assert_eq!(thread_ids(&active), vec![active_thread_id]);
        assert_eq!(thread_ids(&archived), vec![archived_thread_id]);
        assert!(archived.items[0].archived_at.is_some());
    }

    async fn create_test_thread(store: &InMemoryThreadStore) -> ThreadId {
    ...
    fn test_list_params() -> ListThreadsParams {
        ListThreadsParams {
            page_size: 10,
            cursor: None,
    ...
    fn thread_ids(page: &ThreadPage) -> Vec<ThreadId> {
...
struct InMemoryThreadStoreState {
    ...
    archived_threads: HashMap<ThreadId, chrono::DateTime<Utc>>,
}
...
impl ThreadStore for InMemoryThreadStore {
    ...
    async fn resume_thread(&self, params: ResumeThreadParams) -> ThreadStoreResult<()> {
        ...
        ensure_thread_is_eligible_for_archived_filter(
            &state,
            params.thread_id,
    ...
    async fn read_thread_by_rollout_path(
        ...
        ensure_thread_is_eligible_for_archived_filter(&state, thread_id, params.include_archived)?;
    ...
    async fn list_threads(&self, params: ListThreadsParams) -> ThreadStoreResult<ThreadPage> {
        ...
        let mut items = state
            .created_threads
            .keys()
            .filter(|thread_id| state.archived_threads.contains_key(thread_id) == params.archived)
            .map(|thread_id| {
    ...
    async fn archive_thread(&self, params: ArchiveThreadParams) -> ThreadStoreResult<()> {
        ...
        state.archived_threads.insert(params.thread_id, Utc::now());
        Ok(())
    }
...
fn stored_thread_from_state(
    ...
    Ok(StoredThread {
        ...
        updated_at: metadata
            ...
            .unwrap_or_else(Utc::now),
        archived_at: state.archived_threads.get(&thread_id).copied(),
        cwd: metadata
        ...
        usage_limits: metadata.and_then(|metadata| metadata.usage_limits.clone()),
        first_user_message: metadata.and_then(|metadata| metadata.first_user_message.clone()),
        history,
    })
}

fn ensure_thread_is_eligible_for_archived_filter(
    state: &InMemoryThreadStoreState,
    thread_id: ThreadId,
    include_archived: bool,
) -> ThreadStoreResult<()> {
    if !include_archived && state.archived_threads.contains_key(&thread_id) {
        return Err(ThreadStoreError::InvalidRequest {
...
fn git_info_from_patch(patch: &ThreadMetadataPatch) -> Option<runtime_protocol::protocol::GitInfo> {
    ...
    Some(runtime_protocol::protocol::GitInfo {
        commit_hash: sha.as_deref().map(git_utils::GitSha::new),
        branch,
