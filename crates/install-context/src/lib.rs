...
const PACKAGE_METADATA_FILENAME: &str = "sprite-package.json";
const PATH_DIRNAME: &str = "sprite-path";
const RELEASES_DIRNAME: &str = "releases";
const RESOURCES_DIRNAME: &str = "sprite-resources";
...
pub struct InstallContext {
    ...
    pub package_layout: Option<SpritePackageLayout>,
}
...
pub enum InstallMethod {
    Standalone {
        ...
        /// such as
        /// `~/.sprite/packages/standalone/releases/0.111.0-x86_64-unknown-linux-musl`.
        /// Package-layout installs use the package root that contains `bin/`,
        /// `sprite-resources/`, and `sprite-path/`.
    ...
    /// A Sprite binary launched through the npm-managed `sprite.js` shim.
    Npm,
    /// A Sprite binary launched through the bun-managed `sprite.js` shim.
    Bun,
    /// A Sprite binary that appears to come from a Homebrew install prefix.
    Brew,
    /// Any other execution environment.
    ///
    /// This commonly covers `cargo run`, app-bundled Sprite binaries, custom
    /// internal launchers, and tests that execute Sprite from an arbitrary path.
    Other,
}
...
impl InstallContext {
    pub fn from_exe(
        ...
        let sprite_home = utils_home_dir::find_sprite_home().ok();
        Self::from_exe_with_sprite_home(
            is_macos,
            current_exe,
            managed_by_npm,
            managed_by_bun,
            sprite_home.as_deref(),
        )
    }

    fn from_exe_with_sprite_home(
        is_macos: bool,
        current_exe: Option<&Path>,
        managed_by_npm: bool,
        managed_by_bun: bool,
        sprite_home: Option<&Path>,
    ) -> Self {
        let package_layout = current_exe.and_then(SpritePackageLayout::from_exe);
        let method = if managed_by_npm {
            InstallMethod::Npm
        } else if managed_by_bun {
            InstallMethod::Bun
        } else if let Some(exe_path) = current_exe {
            install_method_from_exe(exe_path, sprite_home, package_layout.as_ref(), is_macos)
        } else {
            InstallMethod::Other
        };
    ...
    pub fn current() -> &'static Self {
        INSTALL_CONTEXT.get_or_init(|| {
            ...
            let managed_by_npm = std::env::var_os("SPRITE_MANAGED_BY_NPM").is_some();
            let managed_by_bun = std::env::var_os("SPRITE_MANAGED_BY_BUN").is_some();
            Self::from_exe(
                cfg!(target_os = "macos"),
...
fn install_method_from_exe(
    exe_path: &Path,
    sprite_home: Option<&Path>,
    package_layout: Option<&SpritePackageLayout>,
    is_macos: bool,
) -> InstallMethod {
    if let Some(standalone_method) =
        standalone_install_method(exe_path, sprite_home, package_layout)
    {
...
fn standalone_install_method(
    ...
    let canonical_sprite_home = canonical_absolute_path(sprite_home?)?;
    ...
    let releases_root = canonical_sprite_home
        .join("packages")
...
mod tests {
    ...
    const TEST_RESOURCE_NAME: &str = "sprite-test-helper";

    #[test]
    fn detects_standalone_install_from_release_layout() -> std::io::Result<()> {
        let sprite_home = tempfile::tempdir()?;
        let release_dir = sprite_home
            .path()
        ...
        let context = InstallContext::from_exe_with_sprite_home(
            /*is_macos*/ false,
            /*current_exe*/ Some(&exe_path),
            /*managed_by_npm*/ false,
            /*managed_by_bun*/ false,
            /*sprite_home*/ Some(sprite_home.path()),
        );
        assert_eq!(
            context,
            InstallContext {
    ...
    fn detects_package_layout_independently_from_install_method() -> std::io::Result<()> {
        ...
        let package_layout = SpritePackageLayout {
    ...
    fn standalone_package_layout_keeps_standalone_install_method() -> std::io::Result<()> {
        ...
        assert_eq!(
            ...
            InstallContext {
                ...
                package_layout: Some(SpritePackageLayout {
                    package_dir: canonical_package_dir,
                    bin_dir: canonical_bin_dir,
                    resources_dir: Some(canonical_resources_dir.clone()),
                    path_dir: Some(canonical_path_dir.clone()),
    ...
    #[test]
    fn standalone_package_rg_falls_back_when_sprite_path_is_missing() -> std::io::Result<()> {
        let package_dir = tempfile::tempdir()?;
        let bin_dir = package_dir.path().join(BIN_DIRNAME);
        fs::create_dir_all(&bin_dir)?;
        fs::write(package_dir.path().join(PACKAGE_METADATA_FILENAME), "{}")?;
    ...
    fn npm_and_bun_take_precedence() {
        let npm_context = InstallContext::from_exe_with_sprite_home(
            /*is_macos*/ false,
            /*current_exe*/ Some(Path::new("/tmp/sprite")),
            /*managed_by_npm*/ true,
            /*managed_by_bun*/ false,
            /*sprite_home*/ None,
        );
        ...
        let bun_context = InstallContext::from_exe_with_sprite_home(
            /*is_macos*/ false,
            /*current_exe*/ Some(Path::new("/tmp/sprite")),
            /*managed_by_npm*/ false,
            /*managed_by_bun*/ true,
    ...
    fn brew_is_detected_on_macos_prefixes() {
        let context = InstallContext::from_exe_with_sprite_home(
            /*is_macos*/ true,
            /*current_exe*/ Some(Path::new("/opt/homebrew/bin/sprite")),
            /*managed_by_npm*/ false,
            /*managed_by_bun*/ false,
            /*sprite_home*/ None,
        );
