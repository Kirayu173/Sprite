...
pub struct ThreadMetadata {
    ...
    pub tokens_used: i64,
    /// The last observed usage-limit/rate snapshot.
...
impl ThreadMetadata {
    ...
    pub fn diff_fields(&self, other: &Self) -> Vec<&'static str> {
        ...
        if self.usage_limits != other.usage_limits {
            diffs.push("usage_limits");
        }
...
impl ThreadRow {
    pub(crate) fn try_from_row(row: &SqliteRow) -> Result<Self> {
        Ok(Self {
            ...
            usage_limits: row.try_get("usage_limits")?,
