...
//! orchestration and rollout scanning live in `sprite-core`.
...
pub use db_diagnostics::DbDiagnostics;
pub use db_diagnostics::DbDiagnosticsHandle;
pub use db_diagnostics::install_process_db_diagnostics;
pub use db_diagnostics::record_backfill_gate;
pub use db_diagnostics::record_fallback;
...
pub const SQLITE_HOME_ENV: &str = "SPRITE_SQLITE_HOME";
...
pub const DB_ERROR_METRIC: &str = "sprite.db.error";
/// Metrics on backfill process. Tags: [status]
pub const DB_METRIC_BACKFILL: &str = "sprite.db.backfill";
/// Metrics on backfill duration. Tags: [status]
pub const DB_METRIC_BACKFILL_DURATION_MS: &str = "sprite.db.backfill.duration_ms";
/// SQLite initialization attempts. Tags: [status, phase, db, error]
pub const DB_INIT_METRIC: &str = "sprite.sqlite.init.count";
/// SQLite initialization latency. Tags: [status, phase, db, error]
pub const DB_INIT_DURATION_METRIC: &str = "sprite.sqlite.init.duration_ms";
/// Rollout fallback attempts. Tags: [caller, reason]
pub const DB_FALLBACK_METRIC: &str = "sprite.sqlite.fallback.count";
