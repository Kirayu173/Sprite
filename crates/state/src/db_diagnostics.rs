...
/// Low-cardinality sink for SQLite startup and fallback diagnostics.
///
/// Implementations should absorb delivery failures locally. Database behavior
/// must not depend on whether diagnostics export succeeds.
pub trait DbDiagnostics: Send + Sync + 'static {
    fn counter(&self, name: &str, inc: i64, tags: &[(&str, &str)]);
    fn record_duration(&self, name: &str, duration: Duration, tags: &[(&str, &str)]);
}

pub type DbDiagnosticsHandle = Arc<dyn DbDiagnostics>;

static PROCESS_DB_DIAGNOSTICS: OnceLock<DbDiagnosticsHandle> = OnceLock::new();

/// Install the process-wide SQLite diagnostics sink.
///
...
pub fn install_process_db_diagnostics(diagnostics: DbDiagnosticsHandle) -> bool {
    if PROCESS_DB_DIAGNOSTICS.set(diagnostics).is_ok() {
        true
    } else {
        debug!("process SQLite diagnostics sink already installed; ignoring duplicate install");
        false
    }
}
...
pub(crate) fn record_init_result<T>(
    diagnostics: Option<&dyn DbDiagnostics>,
    db: DbKind,
    phase: &'static str,
    duration: Duration,
    ...
    record_counter(diagnostics, DB_INIT_METRIC, &tags);
    record_duration(diagnostics, DB_INIT_DURATION_METRIC, duration, &tags);
}
...
fn record_counter(diagnostics: Option<&dyn DbDiagnostics>, name: &str, tags: &[(&str, &str)]) {
    if let Some(diagnostics) = resolve_diagnostics(diagnostics) {
        diagnostics.counter(name, /*inc*/ 1, tags);
    }
}
...
fn record_duration(
    ...
    if let Some(diagnostics) = resolve_diagnostics(diagnostics) {
        diagnostics.record_duration(name, duration, tags);
    }
}

fn resolve_diagnostics(diagnostics: Option<&dyn DbDiagnostics>) -> Option<&dyn DbDiagnostics> {
    diagnostics.or_else(|| PROCESS_DB_DIAGNOSTICS.get().map(AsRef::as_ref))
}
