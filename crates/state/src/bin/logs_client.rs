...
#[derive(Debug, Parser)]
#[command(name = "sprite-state-logs")]
#[command(about = "Tail Sprite logs from the dedicated logs SQLite DB with simple filters")]
struct Args {
    /// Path to SPRITE_HOME. Defaults to $SPRITE_HOME or ~/.sprite.
    #[arg(long, env = "SPRITE_HOME")]
    sprite_home: Option<PathBuf>,

    /// Direct path to the logs SQLite database. Overrides --sprite-home.
    #[arg(long)]
    db: Option<PathBuf>,
...
async fn main() -> anyhow::Result<()> {
    ...
    let sprite_home = db_path
        .parent()
        .map(ToOwned::to_owned)
        .unwrap_or_else(|| PathBuf::from("."));
    let runtime = StateRuntime::init(sprite_home, "logs-client".to_string()).await?;
...
fn resolve_db_path(args: &Args) -> anyhow::Result<PathBuf> {
    ...
    let sprite_home = args.sprite_home.clone().unwrap_or_else(default_sprite_home);
    Ok(state::logs_db_path(sprite_home.as_path()))
}

fn default_sprite_home() -> PathBuf {
    if let Some(home) = home_dir() {
        return home.join(".sprite");
    }
    PathBuf::from(".sprite")
}
...
mod tests {
    ...
    fn log_level_rejects_aliases_and_unknown_values() {
        assert!(Args::try_parse_from(["sprite-state-logs", "--level", "warning"]).is_err());
        assert!(Args::try_parse_from(["sprite-state-logs", "--level", "err"]).is_err());
        assert!(Args::try_parse_from(["sprite-state-logs", "--level", "warn,error"]).is_err());
    }

    #[test]
    fn log_level_accepts_canonical_values_case_insensitively() {
        let args = Args::try_parse_from(["sprite-state-logs", "--level", "WARN"])
