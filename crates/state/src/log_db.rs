...
//!
//! # async fn example(state_db: std::sync::Arc<state::StateRuntime>) {
    ...
    S: tracing::Subscriber + for<'a> LookupSpan<'a>,
{
    ...
    fn on_event(&self, event: &Event<'_>, ctx: tracing_subscriber::layer::Context<'_, S>) {
        ...
        // were over 30% of retained logs in measured high-fanout Sprite environments.
        ...
        let local_log_body = format_local_log_body(event, &ctx);
        ...
        let entry = LogEntry {
            ...
            local_log_body: Some(local_log_body),
            thread_id,
...
where
    S: tracing::Subscriber + for<'a> LookupSpan<'a>,
{
    let mut local_log_body = String::new();
    if let Some(scope) = ctx.event_scope(event) {
        for span in scope.from_root() {
            let extensions = span.extensions();
            if let Some(log_context) = extensions.get::<SpanLogContext>() {
                local_log_body.push_str(&log_context.name);
                if !log_context.formatted_fields.is_empty() {
                    local_log_body.push('{');
                    local_log_body.push_str(&log_context.formatted_fields);
                    local_log_body.push('}');
                }
            } else {
                local_log_body.push_str(span.metadata().name());
            }
            local_log_body.push(':');
        }
        if !local_log_body.is_empty() {
            local_log_body.push(' ');
        }
    }
    local_log_body.push_str(&format_fields(event));
    local_log_body
}
...
mod tests {
    ...
    fn temp_sprite_home() -> std::path::PathBuf {
        std::env::temp_dir().join(format!("sprite-state-log-db-{}", Uuid::new_v4()))
    }
    ...
    fn test_entry(message: &str) -> LogEntry {
        LogEntry {
            ...
            local_log_body: Some(message.to_string()),
    ...
    #[tokio::test]
    async fn sqlite_local_logs_match_local_formatter_shape() {
        let sprite_home = temp_sprite_home();
        ...
        tracing::info_span!("local-log-thread", thread_id = "thread-1", turn = 1).in_scope(|| {
        ...
        let local_logs = writer.snapshot();
