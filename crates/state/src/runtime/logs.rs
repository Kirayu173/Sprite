...
impl StateRuntime {
    ...
    pub async fn insert_logs(&self, entries: &[LogEntry]) -> anyhow::Result<()> {
        ...
        let mut builder = QueryBuilder::<Sqlite>::new(
            "INSERT INTO logs (ts, ts_nanos, level, target, local_log_body, thread_id, process_uuid, module_path, file, line, estimated_bytes) ",
        );
        builder.push_values(entries, |mut row, entry| {
            let local_log_body = entry.local_log_body.as_ref().or(entry.message.as_ref());
            // Keep about 10 MiB of reader-visible log content per partition.
            // Both `query_logs` and `local log export` read the persisted
            // `local_log_body`, while `LogEntry.message` is only a write-time
            // fallback for callers that still populate the old field.
            let estimated_bytes = local_log_body.map_or(0, String::len) as i64
...
"#,
    ...
    pub async fn query_logs(&self, query: &LogQuery) -> anyhow::Result<Vec<LogRow>> {
        let mut builder = QueryBuilder::<Sqlite>::new(
            "SELECT id, ts, ts_nanos, level, target, local_log_body AS message, thread_id, process_uuid, file, line FROM logs WHERE 1 = 1",
        );
    ...
    /// Query local logs for a set of threads, capped to the SQLite retention budget.
    pub async fn query_local_logs_for_threads(
        &self,
        thread_ids: &[&str],
...
local_logs AS (
    SELECT ts, ts_nanos, level, local_log_body, estimated_bytes, id
    FROM logs
    WHERE local_log_body IS NOT NULL AND (
...
SELECT ts, ts_nanos, level, local_log_body
FROM bounded_local_logs
...
"#,
        ...
        for row in rows {
            let line = format_local_log_line(row.ts, row.ts_nanos, &row.level, &row.local_log_body);
    ...
    /// Query per-thread local logs, capped to the per-thread SQLite retention budget.
    pub async fn query_local_logs(&self, thread_id: &str) -> anyhow::Result<Vec<u8>> {
        self.query_local_logs_for_threads(&[thread_id]).await
    }
...
struct LocalLogRow {
    ...
    ts_nanos: i64,
    level: String,
    local_log_body: String,
}

fn format_local_log_line(ts: i64, ts_nanos: i64, level: &str, local_log_body: &str) -> String {
    ...
    let mut line = format!("{timestamp} {level:>5} {local_log_body}");
    if !line.ends_with('\n') {
        line.push('\n');
    }
    line
...
fn push_log_filters<'a>(builder: &mut QueryBuilder<'a, Sqlite>, query: &'a LogQuery) {
    ...
    if let Some(search) = query.search.as_ref() {
        builder.push(" AND INSTR(COALESCE(local_log_body, ''), ");
...
mod tests {
    ...
    use super::format_local_log_line;
    ...
    async fn insert_logs_use_dedicated_log_database() {
        ...
        let runtime = StateRuntime::init(sprite_home.clone(), "test-provider".to_string())
            .await
        ...
        runtime
            .insert_logs(&[LogEntry {
                ...
                local_log_body: Some("dedicated-log-db".to_string()),
        ...
        let logs_count = log_row_count(logs_db_path(sprite_home.as_path()).as_path()).await;
    ...
    #[tokio::test]
    async fn init_migrates_message_only_logs_db_to_local_log_body_schema() {
        let sprite_home = unique_temp_dir();
        tokio::fs::create_dir_all(&sprite_home)
            .await
            .expect("create sprite home");
        let logs_path = logs_db_path(sprite_home.as_path());
    ...
    async fn init_configures_logs_db_with_incremental_auto_vacuum() {
        let sprite_home = unique_temp_dir();
        let _runtime = StateRuntime::init(sprite_home.clone(), "test-provider".to_string())
            .await
            .expect("initialize runtime");

        let pool = open_db_pool(logs_db_path(sprite_home.as_path()).as_path()).await;
    ...
    #[test]
    fn format_local_log_line_matches_local_formatter_shape() {
        assert_eq!(
            format_local_log_line(
                /*ts*/ 1,
    ...
    #[test]
    fn format_local_log_line_preserves_existing_trailing_newline() {
        assert_eq!(
            format_local_log_line(
                /*ts*/ 1,
                /*ts_nanos*/ 123_456_000,
        ...
        runtime
            .insert_logs(&[
                LogEntry {
                    ...
                    local_log_body: Some("foo=1 alpha".to_string()),
                    thread_id: Some("thread-1".to_string()),
                    process_uuid: None,
                    file: Some("main.rs".to_string()),
                    line: Some(42),
                ...
                LogEntry {
                    ...
                    local_log_body: Some("foo=2 alphabet".to_string()),
                    thread_id: Some("thread-1".to_string()),
                    process_uuid: None,
                    file: Some("main.rs".to_string()),
                    line: Some(43),
        ...
        runtime
            .insert_logs(&[
                LogEntry {
                    ...
                    local_log_body: Some("trace-row".to_string()),
                    thread_id: None,
                    process_uuid: None,
                    file: Some("main.rs".to_string()),
                    line: Some(1),
                ...
                LogEntry {
                    ...
                    local_log_body: Some("info-row".to_string()),
                    thread_id: None,
                    process_uuid: None,
                    file: Some("main.rs".to_string()),
                    line: Some(2),
                ...
                LogEntry {
                    ...
                    local_log_body: Some("warn-row".to_string()),
                    thread_id: None,
                    process_uuid: None,
                    file: Some("main.rs".to_string()),
                    line: Some(3),
                ...
                LogEntry {
                    ...
                    local_log_body: Some("error-row".to_string()),
                    thread_id: None,
                    process_uuid: None,
                    file: Some("main.rs".to_string()),
                    line: Some(4),
        ...
        runtime
            .insert_logs(&[
                LogEntry {
                    ...
                    local_log_body: Some(six_mebibytes.clone()),
                    thread_id: Some("thread-1".to_string()),
                    process_uuid: Some("proc-1".to_string()),
                    file: Some("main.rs".to_string()),
                    line: Some(1),
    ...
    async fn insert_logs_prunes_single_thread_row_when_it_exceeds_size_limit() {
        ...
        runtime
            .insert_logs(&[LogEntry {
                ...
                target: "cli".to_string(),
                message: Some("small".to_string()),
                local_log_body: Some(eleven_mebibytes),
    ...
    #[tokio::test]
    async fn query_local_logs_returns_newest_lines_within_limit_in_order() {
        let sprite_home = unique_temp_dir();
        let runtime = StateRuntime::init(sprite_home.clone(), "test-provider".to_string())
            .await
            .expect("initialize runtime");
        ...
        let bytes = runtime
            .query_local_logs("thread-1")
            .await
            .expect("query local logs");

        assert_eq!(
            String::from_utf8(bytes).expect("valid utf-8"),
            [
                format_local_log_line(/*ts*/ 1, /*ts_nanos*/ 0, "INFO", "alpha"),
                format_local_log_line(/*ts*/ 2, /*ts_nanos*/ 0, "INFO", "bravo"),
                format_local_log_line(/*ts*/ 3, /*ts_nanos*/ 0, "INFO", "charlie"),
            ]
            .concat()
        );
    ...
    #[tokio::test]
    async fn query_local_logs_excludes_oversized_newest_row() {
        let sprite_home = unique_temp_dir();
        let runtime = StateRuntime::init(sprite_home.clone(), "test-provider".to_string())
            .await
            .expect("initialize runtime");
        ...
        let bytes = runtime
            .query_local_logs("thread-oversized")
            .await
            .expect("query local logs");
    ...
    #[tokio::test]
    async fn query_local_logs_includes_threadless_rows_from_same_process() {
        let sprite_home = unique_temp_dir();
        let runtime = StateRuntime::init(sprite_home.clone(), "test-provider".to_string())
            .await
            .expect("initialize runtime");
        ...
        assert_eq!(
            ...
            [
                ...
                format_local_log_line(/*ts*/ 2, /*ts_nanos*/ 0, "INFO", "thread-scoped"),
                format_local_log_line(
                    /*ts*/ 3,
                    /*ts_nanos*/ 0,
                    "INFO",
    ...
    #[tokio::test]
    async fn query_local_logs_excludes_threadless_rows_from_prior_processes() {
        let sprite_home = unique_temp_dir();
        let runtime = StateRuntime::init(sprite_home.clone(), "test-provider".to_string())
            .await
            .expect("initialize runtime");
    ...
    #[tokio::test]
    async fn query_local_logs_keeps_newest_suffix_across_thread_and_threadless_logs() {
        let sprite_home = unique_temp_dir();
        let runtime = StateRuntime::init(sprite_home.clone(), "test-provider".to_string())
            .await
            .expect("initialize runtime");
    ...
    #[tokio::test]
    async fn query_local_logs_for_threads_merges_requested_threads_and_threadless_rows() {
        let sprite_home = unique_temp_dir();
        let runtime = StateRuntime::init(sprite_home.clone(), "test-provider".to_string())
            .await
            .expect("initialize runtime");
        ...
        let bytes = runtime
            .query_local_logs_for_threads(&["thread-1", "thread-2"])
            .await
            .expect("query local logs");
        ...
        assert_eq!(
            String::from_utf8(bytes).expect("valid utf-8"),
            [
                format_local_log_line(/*ts*/ 1, /*ts_nanos*/ 0, "INFO", "thread-1"),
                format_local_log_line(/*ts*/ 2, /*ts_nanos*/ 0, "INFO", "thread-2"),
                format_local_log_line(
                    /*ts*/ 3,
                    /*ts_nanos*/ 0,
                    "INFO",
    ...
    #[tokio::test]
    async fn query_local_logs_for_threads_returns_empty_for_empty_thread_list() {
        let sprite_home = unique_temp_dir();
        let runtime = StateRuntime::init(sprite_home.clone(), "test-provider".to_string())
            .await
            .expect("initialize runtime");
