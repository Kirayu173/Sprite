...
ON CONFLICT(id) DO NOTHING
        ...
        .bind(
            metadata
                .thread_source
                .map(runtime_protocol::protocol::ThreadSource::as_str),
        )
        ...
        .bind(usage_limits_json(metadata.usage_limits.as_ref()))
...
fn usage_limits_json(usage_limits: Option<&String>) -> Option<&str> {
    usage_limits.map(String::as_str)
}
...
mod tests {
    ...
    use runtime_protocol::protocol::GitInfo;
    use runtime_protocol::protocol::SessionMeta;
    use runtime_protocol::protocol::SessionMetaLine;
    use runtime_protocol::protocol::SessionSource;
    use runtime_protocol::protocol::UsageLimitKind;
    use runtime_protocol::protocol::UsageLimitSnapshot;
    use runtime_protocol::protocol::UsageLimitWindow;
    ...
    async fn upsert_thread_keeps_creation_memory_mode_for_existing_rows() {
        ...
        let mut metadata = test_thread_metadata(&sprite_home, thread_id, sprite_home.clone());
    ...
    async fn list_threads_filters_by_cwd() {
        ...
        let first_cwd = sprite_home.join("first");
        let second_cwd = sprite_home.join("second");
        let other_cwd = sprite_home.join("other");
        ...
        for (thread_id, cwd, updated_at) in [
            ...
            let mut metadata = test_thread_metadata(&sprite_home, thread_id, cwd);
            metadata.updated_at =
    ...
    async fn apply_rollout_items_restores_memory_mode_from_session_meta() {
        ...
        let metadata = test_thread_metadata(&sprite_home, thread_id, sprite_home.clone());
    ...
    async fn apply_rollout_items_preserves_existing_git_branch_and_fills_missing_git_fields() {
        ...
        let items = vec![RolloutItem::SessionMeta(SessionMetaLine {
            ...
            git: Some(GitInfo {
                commit_hash: Some(git_utils::GitSha::new("rollout-sha")),
                branch: Some("rollout-branch".to_string()),
                repository_url: Some("git@example.com:openai/sprite.git".to_string()),
            }),
        })];
        ...
        assert_eq!(
            ...
            Some("git@example.com:openai/sprite.git")
        );
    }
    ...
    async fn upsert_thread_preserves_existing_git_fields_atomically() {
        ...
        metadata.git_branch = Some("sqlite-branch".to_string());
        metadata.git_origin_url = Some("git@example.com:openai/sprite.git".to_string());
    ...
    async fn update_thread_git_info_preserves_newer_non_git_metadata() {
        ...
        let updated = runtime
            .update_thread_git_info(
                ...
                Some(Some("git@example.com:openai/sprite.git")),
            )
            .await
    ...
    async fn insert_thread_if_absent_preserves_existing_metadata() {
        ...
        let mut existing = test_thread_metadata(&sprite_home, thread_id, sprite_home.clone());
        ...
        let mut fallback = test_thread_metadata(&sprite_home, thread_id, sprite_home.clone());
    ...
    async fn thread_updated_at_uses_unique_epoch_millis_and_reads_legacy_seconds() {
        ...
        let mut first = test_thread_metadata(&sprite_home, first_id, sprite_home.clone());
        first.updated_at = updated_at;
        let mut second = test_thread_metadata(&sprite_home, second_id, sprite_home.clone());
        ...
        let mut older = test_thread_metadata(&sprite_home, older_id, sprite_home.clone());
    ...
    async fn apply_rollout_items_uses_override_updated_at_when_provided() {
        ...
        let items = vec![RolloutItem::EventMsg(EventMsg::TokenCount(
            runtime_protocol::protocol::TokenCountEvent {
                info: Some(runtime_protocol::protocol::TokenUsageInfo {
                    total_token_usage: runtime_protocol::protocol::TokenUsage {
                        input_tokens: 0,
                    ...
                    last_token_usage: runtime_protocol::protocol::TokenUsage::default(),
    ...
    #[tokio::test]
    async fn apply_rollout_items_persists_usage_limit_snapshot() {
        let sprite_home = unique_temp_dir();
        let runtime = StateRuntime::init(sprite_home.clone(), "test-provider".to_string())
            .await
            .expect("state db should initialize");
        ...
        let snapshot = UsageLimitSnapshot {
            ...
            primary: Some(UsageLimitWindow {
                ...
                resets_at: Some(1_700_010_000),
            }),
            secondary: None,
            kind: Some(UsageLimitKind::WorkspaceLimitReached),
        };
        ...
        runtime
            .apply_rollout_items(
                ...
                &[RolloutItem::EventMsg(EventMsg::TokenCount(
                    runtime_protocol::protocol::TokenCountEvent {
                        info: None,
                        usage_limits: Some(snapshot.clone()),
                    },
                ))],
                /*new_thread_memory_mode*/ None,
                None,
        ...
        assert_eq!(
            persisted
                .usage_limits
                .as_deref()
                .map(serde_json::from_str::<UsageLimitSnapshot>)
                .transpose()
                .expect("usage limits json"),
            Some(snapshot)
        );
    }
    ...
    async fn thread_spawn_edges_track_directional_status() {
        let sprite_home = unique_temp_dir();
        let runtime = StateRuntime::init(sprite_home, "test-provider".to_string())
            .await
            .expect("state db should initialize");
