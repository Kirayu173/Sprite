...
fn blocking_set_model_top_level() {
    ...
    let contents = std::fs::read_to_string(sprite_home.join(CONFIG_TOML_FILE)).expect("read config");
...
fn keymap_bindings_edit_writes_multiple_bindings_as_array() {
    ...
    let raw = std::fs::read_to_string(sprite_home.join(CONFIG_TOML_FILE)).expect("read config");
...
fn blocking_set_model_writes_through_symlink_chain() {
    ...
    let link_path = sprite_home.join("config-link.toml");
...
fn blocking_set_model_replaces_symlink_on_cycle() {
    let tmp = tempdir().expect("tmpdir");
    let sprite_home = tmp.path();
    let link_a = sprite_home.join("a.toml");
    let link_b = sprite_home.join("b.toml");
    let config_path = sprite_home.join(CONFIG_TOML_FILE);
...
"#;
    std::fs::write(sprite_home.join(CONFIG_TOML_FILE), original).expect("seed config");
    ...
    let updated = std::fs::read_to_string(sprite_home.join(CONFIG_TOML_FILE)).expect("read config");
...
fn blocking_replace_mcp_servers_serializes_tool_approval_overrides() {
    ...
    apply_blocking(sprite_home, &[ConfigEdit::ReplaceMcpServers(servers)]).expect("persist");
...
fn blocking_clear_path_noop_when_missing() {
    ...
    assert!(
        !sprite_home.join(CONFIG_TOML_FILE).exists(),
...
async fn async_builder_set_model_persists() {
    let tmp = tempdir().expect("tmpdir");
    let sprite_home = tmp.path().to_path_buf();
...
"#;
    ...
    let mut contents =
        std::fs::read_to_string(sprite_home.join(CONFIG_TOML_FILE)).expect("read config");
...
"#;
    ...
    contents = std::fs::read_to_string(sprite_home.join(CONFIG_TOML_FILE)).expect("read config");
