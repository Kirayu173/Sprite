...
pub fn apply_blocking(sprite_home: &Path, edits: &[ConfigEdit]) -> anyhow::Result<()> {
...
///
pub async fn apply(sprite_home: &Path, edits: Vec<ConfigEdit>) -> anyhow::Result<()> {
    let sprite_home = sprite_home.to_path_buf();
    let config_path = sprite_home.join(CONFIG_TOML_FILE);
...
impl ConfigEditsBuilder {
    ...
    pub fn for_config(config: &crate::config::Config) -> Self {
        let config_path = config
            ...
            .unwrap_or_else(|| config.sprite_home.join(CONFIG_TOML_FILE).to_path_buf());
