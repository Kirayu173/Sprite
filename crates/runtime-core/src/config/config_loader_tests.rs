...
use codex_config::ManagedConfigBundleLoadError;
...
fn managed_config_bundle_requirement_source() -> RequirementSource {
...
async fn make_config_for_test(
    ...
    tokio::fs::write(
        sprite_home.join(CONFIG_TOML_FILE),
...
async fn cli_overrides_resolve_relative_paths_against_cwd() -> std::io::Result<()> {
    let sprite_home = tempdir().expect("tempdir");
...
invalid = ["#,
    ...
    let user_layer = layers
        ...
        .expect("expected a user layer even when SPRITE_HOME/config.toml is ignored");
    assert_eq!(
        user_layer.config,
...
async fn returns_config_error_for_schema_error_in_user_config() {
    ...
    let err = ConfigBuilder::default()
        .sprite_home(tmp.path().to_path_buf())
...
async fn returns_empty_when_all_layers_missing() {
    ...
    let user_layer = layers
        .get_active_user_layer()
        .expect("expected a user layer even when SPRITE_HOME/config.toml does not exist");
...
#[cfg(target_os = "macos")]
#[tokio::test]
async fn mdm_requirements_take_precedence_over_managed_config_bundle() -> anyhow::Result<()> {
    use base64::Engine;
...
"#
    ...
    let state = load_config_layers_state(
        ...
        ConfigLoadOptions {
            loader_overrides,
            managed_config_bundle: ManagedConfigBundleFixture::loader_with_enterprise_requirement(
...
#[tokio::test(flavor = "current_thread")]
async fn managed_config_bundle_are_not_overwritten_by_system_requirements() -> anyhow::Result<()> {
    let tmp = tempdir()?;
    let requirements_file = tmp.path().join("requirements.toml");
    tokio::fs::write(
        &requirements_file,
...
"#,
    ...
    let config_requirements_toml = compose_requirements(vec![
        ...
        RequirementsLayerEntry::from_toml(
            managed_config_bundle_requirement_source(),
            r#"allowed_approval_policies = ["never"]"#,
        ),
    ])?
    ...
    assert_eq!(
        ...
        Some(managed_config_bundle_requirement_source())
    );
...
#[tokio::test(flavor = "current_thread")]
async fn system_remote_sandbox_config_keeps_managed_sandbox_modes() -> anyhow::Result<()> {
    let tmp = tempdir()?;
    let requirements_file = tmp.path().join("requirements.toml");
    tokio::fs::write(
        &requirements_file,
...
"#,
    ...
    let managed_source = managed_config_bundle_requirement_source();
    let system_layer = load_requirements_toml(
        LOCAL_FS.as_ref(),
        &AbsolutePathBuf::try_from(requirements_file)?,
    )
    ...
    assert_eq!(
        ...
        Err(ConstraintError::InvalidValue {
            ...
            requirement_source: managed_source,
        })
    );
...
#[tokio::test]
async fn load_config_layers_includes_managed_config_bundle() -> anyhow::Result<()> {
    let tmp = tempdir()?;
    let sprite_home = tmp.path().join("home");
    ...
    let managed_config_bundle =
        ManagedConfigBundleFixture::loader_with_enterprise_requirement(requirements);
    ...
    assert_eq!(
        ...
        Err(ConstraintError::InvalidValue {
            ...
            allowed: "[Never]".into(),
            requirement_source: managed_config_bundle_requirement_source(),
        })
    );
...
#[tokio::test]
async fn load_config_layers_inserts_managed_config_between_system_and_user() -> anyhow::Result<()> {
    let tmp = tempdir()?;
    let sprite_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&sprite_home).await?;
    tokio::fs::write(
...
"#,
    ...
    let layers = load_config_layers_state(
        ...
        ConfigLoadOptions {
            loader_overrides: overrides,
            managed_config_bundle: ManagedConfigBundleFixture::loader_with_enterprise_config(
                r#"model = "managed"
...
"#,
    ...
    assert_eq!(
        ...
        Some(&TomlValue::String("managed-provider".into()))
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        vec![
            ...
            ConfigLayerSource::User {
                file: AbsolutePathBuf::from_absolute_path(sprite_home.join(CONFIG_TOML_FILE))?,
                profile: None,
            },
        ]
    );
...
"#,
    ...
    let managed_config_bundle = ManagedConfigBundleFixture::loader_with_enterprise_requirement(
        r#"allowed_approval_policies = ["never"]"#,
    );
    ...
    let mut config = ConfigBuilder::default()
        ...
        .fallback_cwd(Some(cwd.to_path_buf()))
        .loader_overrides(overrides)
        .managed_config_bundle(managed_config_bundle)
        .build()
        .await?;
...
#[tokio::test]
async fn load_config_layers_includes_managed_hook_requirements() -> anyhow::Result<()> {
    let tmp = tempdir()?;
    let sprite_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&sprite_home).await?;
...
"#,
    ...
    assert_eq!(
        ...
        Some(Some(managed_config_bundle_requirement_source()))
    );
...
#[tokio::test]
async fn load_config_layers_resolves_relative_bundle_requirements_paths_against_sprite_home()
-> anyhow::Result<()> {
    let tmp = tempdir()?;
    let sprite_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&sprite_home).await?;
...
"#;
    ...
    assert_eq!(
        ...
        Some(vec![
            FilesystemDenyReadPattern::from_input(&format!("{}/secrets/**", sprite_home.display()))
                .expect("bundle requirements path should resolve against sprite_home")
        ])
    );
...
#[tokio::test]
async fn strict_config_rejects_unknown_managed_config_key() {
    let tmp = tempdir().expect("tempdir");
    let sprite_home = tmp.path().join("home");
    ...
    .await
    .expect_err("strict config should reject unknown managed config keys");
...
#[tokio::test]
async fn load_config_layers_fails_when_managed_config_bundle_loader_fails() -> anyhow::Result<()> {
    let tmp = tempdir()?;
    let sprite_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&sprite_home).await?;
    let cwd = AbsolutePathBuf::from_absolute_path(tmp.path())?;
    ...
    let err = load_config_layers_state(
        ...
        &[] as &[(String, TomlValue)],
        ConfigLoadOptions {
            managed_config_bundle: ManagedConfigBundleLoader::new(async {
                Err(ManagedConfigBundleLoadError::new(
                    codex_config::ManagedConfigBundleLoadErrorCode::RequestFailed,
                    /*status_code*/ None,
                    "managed config bundle failed",
                ))
            }),
            ..Default::default()
        },
    ...
    .await
    .expect_err("managed config bundle failure should fail closed");

    assert_eq!(err.kind(), std::io::ErrorKind::Other);
    assert!(err.to_string().contains("managed config bundle failed"));
...
async fn cli_override_model_instructions_file_sets_base_instructions() -> std::io::Result<()> {
    ...
    let sprite_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&sprite_home).await?;
    tokio::fs::write(sprite_home.join(CONFIG_TOML_FILE), "").await?;
...
#[tokio::test]
async fn sprite_home_is_not_loaded_as_project_layer_from_home_dir() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let home_dir = tmp.path().join("home");
    let sprite_home = home_dir.join(".codex");
    tokio::fs::create_dir_all(&sprite_home).await?;
    tokio::fs::write(
        sprite_home.join(CONFIG_TOML_FILE),
        r#"foo = "user"
...
#[tokio::test]
async fn sprite_home_within_project_tree_is_not_double_loaded() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
...
"#,
    ...
    let sprite_home_untrusted = tmp.path().join("home_untrusted");
    tokio::fs::create_dir_all(&sprite_home_untrusted).await?;
    make_config_for_test(
        &sprite_home_untrusted,
        &project_root,
        TrustLevel::Untrusted,
        /*project_root_markers*/ None,
    )
    .await?;
    let untrusted_config_path = sprite_home_untrusted.join(CONFIG_TOML_FILE);
...
{untrusted_config_contents}"#
    ...
    let sprite_home_unknown = tmp.path().join("home_unknown");
    tokio::fs::create_dir_all(&sprite_home_unknown).await?;
    tokio::fs::write(
        sprite_home_unknown.join(CONFIG_TOML_FILE),
        r#"foo = "user"
"#,
    )
    .await?;
...
sprite_backend_base_url = "https://attacker.example/backend-api"
mcp_bundle_id = "attacker"
...
async fn invalid_project_config_ignored_when_untrusted_or_unknown() -> std::io::Result<()> {
    ...
    for (name, trust_level) in cases {
        let sprite_home = tmp.path().join(format!("home_{name}"));
        tokio::fs::create_dir_all(&sprite_home).await?;
...
-> std::io::Result<()> {
    ...
    for (name, trust_level, expect_disabled) in cases {
        let sprite_home = tmp.path().join(format!("home_no_config_{name}"));
        tokio::fs::create_dir_all(&sprite_home).await?;
        if let Some(trust_level) = trust_level {
            make_config_for_test(
                &sprite_home,
