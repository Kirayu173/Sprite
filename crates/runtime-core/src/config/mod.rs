...
use codex_config::ManagedConfigBundleLoader;
...
use codex_features::McpPathOverrideConfigToml;
...
use codex_features::MultiAgentConfigToml;
...
pub(crate) async fn test_config() -> Config {
    let sprite_home = tempfile::tempdir().expect("create temp dir");
    Config::load_from_base_config_with_overrides(
        ConfigToml::default(),
        ConfigOverrides::default(),
        AbsolutePathBuf::from_absolute_path(sprite_home.path()).expect("temp dir should resolve"),
    )
    .await
...
pub struct Config {
    ...
    /// file: SPRITE_HOME/.credentials.json
    ...
    /// overridden by the `SPRITE_HOME` environment variable).
    pub sprite_home: AbsolutePathBuf,

    /// Directory where Codex stores the SQLite state DB.
    pub sqlite_home: PathBuf,

    /// Directory where Codex writes log files (defaults to `$SPRITE_HOME/log`).
    pub log_dir: PathBuf,
    ...
    pub config_lock_allow_sprite_version_mismatch: bool,
    ...
    /// Base URL for requests to Sprite backend (as opposed to the OpenAI API).
    pub sprite_backend_base_url: String,

    /// Optional path override for the host-owned apps MCP server.
    pub mcp_path_override: Option<String>,

    /// Optional bundle id forwarded to the host-owned apps MCP server.
    ...
    /// When set, restricts Sprite backend login to one or more workspace identifiers.
    pub forced_sprite_backend_workspace_id: Option<Vec<String>>,
...
pub struct ConfigBuilder {
    ...
    strict_config: bool,
    managed_config_bundle: ManagedConfigBundleLoader,
    thread_config_loader: Option<Arc<dyn ThreadConfigLoader>>,
    fallback_cwd: Option<PathBuf>,
}

impl ConfigBuilder {
    pub fn sprite_home(mut self, sprite_home: PathBuf) -> Self {
        self.sprite_home = Some(sprite_home);
        self
    }
    ...
    pub fn managed_config_bundle(mut self, managed_config_bundle: ManagedConfigBundleLoader) -> Self {
        self.managed_config_bundle = managed_config_bundle;
        self
    }
    ...
    async fn build_inner(self) -> std::io::Result<Config> {
        ...
        let sprite_home = match sprite_home {
            Some(sprite_home) => AbsolutePathBuf::from_absolute_path(sprite_home)?,
            None => find_sprite_home()?,
        };
        ...
        {
            let allow_sprite_version_mismatch = config_lock_settings
                .and_then(|config_lock| config_lock.allow_sprite_version_mismatch)
                .unwrap_or(false);
            ...
            config.config_lock_allow_sprite_version_mismatch = allow_sprite_version_mismatch;
...
impl Config {
    pub(crate) fn multi_agent_version_from_features(&self) -> MultiAgentVersion {
        if self.features.enabled(Feature::MultiAgent) {
    ...
    pub(crate) fn validate_multi_agent_config(&self) -> std::io::Result<()> {
        if self.features.enabled(Feature::MultiAgent) && self.agent_max_threads.is_some() {
    ...
    pub async fn to_mcp_config(
        ...
        McpConfig {
            sprite_backend_base_url: self.sprite_backend_base_url.clone(),
            mcp_path_override: self.mcp_path_override.clone(),
            mcp_bundle_id: self.mcp_bundle_id.clone(),
            sprite_home: self.sprite_home.to_path_buf(),
    ...
    pub async fn rebuild_preserving_session_layers(
        ...
        Self::load_config_with_layer_stack(
            ...
            refreshed_config.sprite_home.clone(),
            config_layer_stack,
        )
        .await
    }
    ...
    pub async fn load_default_with_cli_overrides(
        ...
        let sprite_home = find_sprite_home()?;
        Self::load_default_with_cli_overrides_for_sprite_home(
            sprite_home.to_path_buf(),
            cli_overrides,
        )
        .await
    }
    ...
    pub async fn load_default_with_cli_overrides_for_sprite_home(
        sprite_home: PathBuf,
        cli_overrides: Vec<(String, TomlValue)>,
    ) -> std::io::Result<Self> {
        ...
        let sprite_home = AbsolutePathBuf::from_absolute_path_checked(sprite_home)?;
        let config_toml = deserialize_config_toml_with_base(merged, &sprite_home)?;
        Self::load_config_with_layer_stack(
            LOCAL_FS.as_ref(),
            config_toml,
            ConfigOverrides::default(),
    ...
    /// ignores user settings. For example, the `sprite exec` subcommand is
...
pub async fn load_config_as_toml_with_cli_and_loader_overrides(
    ...
    load_config_as_toml_with_cli_and_load_options(sprite_home, cwd, cli_overrides, loader_overrides)
        .await
}
...
pub async fn load_config_as_toml_with_cli_and_load_options(
    ...
    let merged_toml = config_layer_stack.effective_config();
    let cfg = deserialize_config_toml_with_base(merged_toml, sprite_home).map_err(|e| {
...
/// Patch `SPRITE_HOME/config.toml` project state to set trust level.
/// Use with caution.
...
pub fn set_default_oss_provider(sprite_home: &Path, provider: &str) -> std::io::Result<()> {
...
fn resolve_multi_agent_config(config_toml: &ConfigToml) -> MultiAgentConfig {
    let base = multi_agent_toml_config(config_toml.features.as_ref());
    let default = MultiAgentConfig::default();
...
fn multi_agent_toml_config(features: Option<&FeaturesToml>) -> Option<&MultiAgentConfigToml> {
    match features?.multi_agent.as_ref()? {
        FeatureToml::Enabled(_) => None,
        FeatureToml::Config(config) => Some(config),
    }
}

fn mcp_path_override_toml_config(
    features: Option<&FeaturesToml>,
) -> Option<&McpPathOverrideConfigToml> {
    match features?.mcp_path_override.as_ref()? {
        FeatureToml::Enabled(_) => None,
        FeatureToml::Config(config) => Some(config),
    }
}
...
fn validate_multi_agent_wait_timeout(label: &str, value: i64) -> std::io::Result<()> {
...
fn validate_multi_agent_tool_namespace(namespace: Option<&str>) -> std::io::Result<()> {
    const LABEL: &str = "features.multi_agent.tool_namespace";
...
impl Config {
    ...
    pub(crate) async fn load_config_with_layer_stack(
        ...
        let memories_root = memory_root(&sprite_home);
        ...
        let multi_agent = resolve_multi_agent_config(&cfg);
        let mcp_path_override = if features.enabled(Feature::McpPathOverride) {
            let base = mcp_path_override_toml_config(cfg.features.as_ref());
        ...
        if multi_agent.max_concurrent_threads_per_session == 0 {
            return Err(std::io::Error::new(
                std::io::ErrorKind::InvalidInput,
                "features.multi_agent.max_concurrent_threads_per_session must be at least 1",
            ));
        }
        validate_multi_agent_wait_timeout(
            "features.multi_agent.min_wait_timeout_ms",
            multi_agent.min_wait_timeout_ms,
        )?;
        validate_multi_agent_wait_timeout(
            "features.multi_agent.max_wait_timeout_ms",
            multi_agent.max_wait_timeout_ms,
        )?;
        validate_multi_agent_wait_timeout(
            "features.multi_agent.default_wait_timeout_ms",
            multi_agent.default_wait_timeout_ms,
        )?;
        if multi_agent.min_wait_timeout_ms > multi_agent.max_wait_timeout_ms {
            return Err(std::io::Error::new(
                std::io::ErrorKind::InvalidInput,
                "features.multi_agent.min_wait_timeout_ms must be at most features.multi_agent.max_wait_timeout_ms",
            ));
        }
        if multi_agent.default_wait_timeout_ms < multi_agent.min_wait_timeout_ms {
            return Err(std::io::Error::new(
                std::io::ErrorKind::InvalidInput,
                "features.multi_agent.default_wait_timeout_ms must be at least features.multi_agent.min_wait_timeout_ms",
            ));
        }
        if multi_agent.default_wait_timeout_ms > multi_agent.max_wait_timeout_ms {
            return Err(std::io::Error::new(
                std::io::ErrorKind::InvalidInput,
                "features.multi_agent.default_wait_timeout_ms must be at most features.multi_agent.max_wait_timeout_ms",
            ));
        }
        validate_multi_agent_tool_namespace(multi_agent.tool_namespace.as_deref())?;
        ...
        let forced_sprite_backend_workspace_id = cfg
            .forced_sprite_backend_workspace_id
            .clone()
        ...
        let log_dir = cfg
            ...
            .unwrap_or_else(|| sprite_home.join("log").to_path_buf());
        let sqlite_home = cfg
            .sqlite_home
            .as_ref()
            .map(AbsolutePathBuf::to_path_buf)
            .or_else(|| resolve_sqlite_home_env(&resolved_cwd))
            .unwrap_or_else(|| sprite_home.to_path_buf());
        ...
        let config = Self {
            ...
            config_lock_allow_sprite_version_mismatch: cfg
                .debug
                .as_ref()
                .and_then(|debug| debug.config_lockfile.as_ref())
                .and_then(|config_lock| config_lock.allow_sprite_version_mismatch)
            ...
            sprite_backend_base_url: cfg
                .sprite_backend_base_url
                .unwrap_or("https://sprite_backend.com/backend-api/".to_string()),
            mcp_path_override,
            mcp_bundle_id: cfg.mcp_bundle_id.clone(),
            realtime_audio: cfg
                .audio
...
/// specified by the `SPRITE_HOME` environment variable. If not set, defaults to
/// `~/.codex`.
///
/// - If `SPRITE_HOME` is set, the value must exist and be a directory. The
///   value will be canonicalized and this function will Err otherwise.
/// - If `SPRITE_HOME` is not set, this function does not verify that the
///   directory exists.
pub fn find_sprite_home() -> std::io::Result<AbsolutePathBuf> {
    codex_utils_home_dir::find_sprite_home()
}
