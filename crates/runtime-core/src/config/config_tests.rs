...
use codex_config::test_support::ManagedConfigBundleFixture;
...
async fn load_config_loads_global_agents_instructions() -> std::io::Result<()> {
    let sprite_home = tempdir()?;
    let global_agents_path = sprite_home.abs().join(DEFAULT_AGENTS_MD_FILENAME);
...
async fn load_config_prefers_global_agents_override_instructions() -> std::io::Result<()> {
    let sprite_home = tempdir()?;
    std::fs::write(
        sprite_home.path().join(DEFAULT_AGENTS_MD_FILENAME),
        "global instructions",
    )?;
    let global_agents_override_path = sprite_home.abs().join(LOCAL_AGENTS_MD_FILENAME);
...
removed_provider_auth_flag = true
...
async fn network_proxy_cli_overrides_merge_toggle_with_proxy_config() -> std::io::Result<()> {
    ...
    let cwd = TempDir::new()?;
    std::fs::write(
        sprite_home.path().join(CONFIG_TOML_FILE),
        r#"
...
"#,
    ...
    let config = ConfigBuilder::without_managed_config_for_tests()
        .sprite_home(sprite_home.path().to_path_buf())
        .cli_overrides(vec![
            (
...
async fn experimental_network_requirements_enable_proxy_without_feature() -> std::io::Result<()> {
    ...
    let config = ConfigBuilder::without_managed_config_for_tests()
        .sprite_home(sprite_home.path().to_path_buf())
        .fallback_cwd(Some(sprite_home.path().to_path_buf()))
        .managed_config_bundle(
            ManagedConfigBundleFixture::loader_with_enterprise_requirement(
                r#"
...
-> std::io::Result<()> {
    ...
    let memories_root = sprite_home.path().join("memories").abs();
    assert!(
        !config
            .permissions
            .file_system_sandbox_policy()
...
-> std::io::Result<()> {
    ...
    let sprite_home = temp_dir.path().join("codex-home");
...
#[tokio::test]
async fn forced_sprite_backend_workspace_id_empty_values_disable_runtime_restriction()
-> std::io::Result<()> {
    let cases: Vec<(&str, &str, Option<Vec<&str>>)> = vec![
        ("unset", "", None),
        ("empty string", r#"forced_sprite_backend_workspace_id = """#, None),
        (
            "whitespace string",
            r#"forced_sprite_backend_workspace_id = "   ""#,
            None,
        ),
        ("empty list", r#"forced_sprite_backend_workspace_id = []"#, None),
        (
            "blank list entries",
            r#"forced_sprite_backend_workspace_id = ["", "  "]"#,
            None,
        ...
        (
            "mixed list entries",
            r#"forced_sprite_backend_workspace_id = ["", " 123e4567-e89b-42d3-a456-426614174000 ", "123e4567-e89b-42d3-a456-426614174001"]"#,
            Some(vec![
    ...
    for (name, toml, expected) in cases {
        let cfg_toml: ConfigToml = toml::from_str(toml)
            .unwrap_or_else(|err| panic!("{name} should parse forced_sprite_backend_workspace_id: {err}"));
        let config = Config::load_from_base_config_with_overrides(
            cfg_toml,
            ConfigOverrides::default(),
            tempdir().expect("tempdir").abs(),
        ...
        assert_eq!(config.forced_sprite_backend_workspace_id, expected, "{name}");
    }
...
async fn rebuild_preserving_session_layers_refreshes_requirements() -> std::io::Result<()> {
    let sprite_home = TempDir::new()?;
    let user_file = AbsolutePathBuf::resolve_path_against_base(CONFIG_TOML_FILE, sprite_home.path());
    let project_dot_codex =
        AbsolutePathBuf::resolve_path_against_base("project/.codex", sprite_home.path());
    ...
    let refreshed_config = Config::load_config_with_layer_stack(
        ...
        refreshed_toml,
        ConfigOverrides {
            cwd: Some(sprite_home.path().to_path_buf()),
            ..Default::default()
        },
        sprite_home.abs(),
...
-> anyhow::Result<()> {
  ...
  "mcpServers": {
    ...
    let config = thread_config
        .rebuild_preserving_session_layers(&refreshed_config)
        .await?;
    let plugins_manager = PluginsManager::new(sprite_home.path().to_path_buf());
...
#[tokio::test]
async fn to_mcp_config_applies_plugin_mcp_managed_config_bundle() -> anyhow::Result<()> {
    let sprite_home = TempDir::new()?;
    let plugin_root = sprite_home
        .path()
        .join("plugins/cache")
...
async fn default_zsh_path_sets_runtime_zsh_path() -> std::io::Result<()> {
    let sprite_home = TempDir::new()?;
    let default_zsh_path = sprite_home.path().join("packaged-zsh");
...
#[tokio::test]
async fn sqlite_home_defaults_to_sprite_home_for_workspace_write() -> std::io::Result<()> {
    let sprite_home = TempDir::new()?;
    let config = Config::load_from_base_config_with_overrides(
        ConfigToml::default(),
        ConfigOverrides {
    ...
    assert_eq!(config.sqlite_home, sprite_home.path().to_path_buf());
...
-> std::io::Result<()> {
    let sprite_home = TempDir::new()?;
    let memories_root = sprite_home.path().join("memories");
    let writable_root = sprite_home.path().join("writable").abs();
    let config = Config::load_from_base_config_with_overrides(
        ConfigToml {
            sandbox_workspace_write: Some(SandboxWorkspaceWrite {
...
async fn managed_config_overrides_oauth_store_mode() -> anyhow::Result<()> {
    let sprite_home = TempDir::new()?;
    let managed_path = sprite_home.path().join("managed_config.toml");
    let config_path = sprite_home.path().join(CONFIG_TOML_FILE);
    ...
    .await?;
    let cfg =
        deserialize_config_toml_with_base(config_layer_stack.effective_config(), sprite_home.path())
            .map_err(|e| {
...
async fn load_global_mcp_servers_returns_empty_if_missing() -> anyhow::Result<()> {
    ...
    let servers = load_global_mcp_servers(sprite_home.path()).await?;
...
async fn replace_mcp_servers_round_trips_entries() -> anyhow::Result<()> {
    ...
    let loaded = load_global_mcp_servers(sprite_home.path()).await?;
    ...
    match &docs.transport {
        McpServerTransportConfig::Stdio {
            ...
            assert_eq!(cwd, &Some(sprite_home.path().to_path_buf()));
        }
...
async fn to_mcp_config_preserves_apps_feature_from_config() -> std::io::Result<()> {
    ...
    config.mcp_path_override = Some("/custom/mcp".to_string());
    config.mcp_bundle_id = Some("tpp".to_string());
    let mcp_config = config.to_mcp_config(&plugins_manager).await;
    assert!(mcp_config.apps_enabled);
    assert_eq!(
        mcp_config.mcp_path_override.as_deref(),
        Some("/custom/mcp")
    );
    assert_eq!(mcp_config.mcp_bundle_id.as_deref(), Some("tpp"));
...
"#,
    ...
    let err = load_global_mcp_servers(sprite_home.path())
        .await
...
async fn set_model_updates_defaults() -> anyhow::Result<()> {
    ...
    ConfigEditsBuilder::new(sprite_home.path())
        .set_model(Some("gpt-5.4"), Some(ReasoningEffort::High))
        .apply()
        .await?;

    let serialized = tokio::fs::read_to_string(sprite_home.path().join(CONFIG_TOML_FILE)).await?;
...
async fn for_config_writes_selected_user_config_file() -> anyhow::Result<()> {
    let sprite_home = TempDir::new()?;
    let base_config = sprite_home.path().join(CONFIG_TOML_FILE);
    let selected_config = sprite_home.path().join("work.config.toml");
...
fn profile_v2_config_path_resolves_validated_names() -> anyhow::Result<()> {
    ...
    assert_eq!(
        resolve_profile_v2_config_path(sprite_home.path(), &profile_name),
        sprite_home.path().join("work.config.toml").abs()
    );
    Ok(())
}
...
impl PrecedenceTestFixture {
    ...
    fn sprite_home(&self) -> AbsolutePathBuf {
        self.sprite_home.abs()
    }
}
...
async fn loads_compact_prompt_from_file() -> std::io::Result<()> {
    let sprite_home = TempDir::new()?;
    let workspace = sprite_home.path().join("workspace");
    ...
    let config =
        Config::load_from_base_config_with_overrides(cfg, overrides, sprite_home.abs()).await?;
...
async fn load_config_rejects_missing_agent_role_config_file() -> std::io::Result<()> {
    let sprite_home = TempDir::new()?;
    let missing_path = sprite_home.path().join("agents").join("researcher.toml");
    let cfg = ConfigToml {
...
async fn agent_role_relative_config_file_resolves_against_config_toml() -> std::io::Result<()> {
    let sprite_home = TempDir::new()?;
    let role_config_path = sprite_home.path().join("agents").join("researcher.toml");
...
"#,
    ...
    let config_layer_stack = codex_config::ConfigLayerStack::new(
        vec![codex_config::ConfigLayerEntry::new(
            codex_app_server_protocol::ConfigLayerSource::User {
                file: sprite_home.path().join(CONFIG_TOML_FILE).abs(),
                profile: None,
            },
            layer_config,
        )],
...
async fn loads_legacy_split_agent_roles_from_config_toml() -> std::io::Result<()> {
    let sprite_home = TempDir::new()?;
    let researcher_path = sprite_home.path().join("agents").join("researcher.toml");
    let reviewer_path = sprite_home.path().join("agents").join("reviewer.toml");
    tokio::fs::create_dir_all(
        researcher_path
            .parent()
            .expect("role config should have a parent directory"),
...
"#
    ...
    let home_agents_dir = sprite_home.path().join("agents");
...
async fn model_catalog_json_loads_from_path() -> std::io::Result<()> {
    let sprite_home = TempDir::new()?;
    let catalog_path = sprite_home.path().join("catalog.json");
...
"#;
    ...
    let sprite_home_temp_dir = TempDir::new().unwrap();

    Ok(PrecedenceTestFixture {
        cwd: cwd_temp_dir,
        sprite_home: sprite_home_temp_dir,
        cfg,
    })
}
...
async fn config_toml_priority_service_tier_uses_priority_request_value() -> std::io::Result<()> {
    ...
    let sprite_home = fixture.sprite_home();
...
fn test_set_default_oss_provider() -> std::io::Result<()> {
    ...
    let result = set_default_oss_provider(sprite_home, "invalid_provider");
...
fn test_set_default_oss_provider_rejects_legacy_ollama_chat_provider() -> std::io::Result<()> {
    ...
    let result = set_default_oss_provider(sprite_home, LEGACY_OLLAMA_CHAT_PROVIDER_ID);
    assert!(result.is_err());
    let error = result.unwrap_err();
    assert_eq!(error.kind(), std::io::ErrorKind::InvalidInput);
    assert!(
...
#[tokio::test]
async fn config_loads_mcp_path_override_from_feature_config() -> std::io::Result<()> {
    let sprite_home = TempDir::new()?;
    let toml = r#"
model = "gpt-5.4"

[features.mcp_path_override]
path = "/custom/mcp"
"#;
    let cfg: ConfigToml =
    ...
    assert_eq!(
        config.mcp_path_override.as_deref(),
        Some("/custom/mcp")
    );
    Ok(())
}
...
#[tokio::test]
async fn config_defaults_enabled_mcp_path_override_to_plugin_service() -> std::io::Result<()> {
    let sprite_home = TempDir::new()?;
    let toml = r#"
model = "gpt-5.4"

[features]
mcp_path_override = true
"#;
    let cfg: ConfigToml =
        toml::from_str(toml).expect("TOML deserialization should succeed for apps MCP feature");
    ...
    assert!(config.features.enabled(Feature::McpPathOverride));
    assert_eq!(config.mcp_path_override.as_deref(), Some("/ps/mcp"));
    Ok(())
}

#[tokio::test]
async fn config_preserves_explicit_mcp_path_override_path() -> std::io::Result<()> {
    let sprite_home = TempDir::new()?;
    let toml = r#"
model = "gpt-5.4"
...
#[tokio::test]
async fn config_loads_mcp_bundle_id_from_toml() -> std::io::Result<()> {
    let sprite_home = TempDir::new()?;
    let toml = r#"
model = "gpt-5.4"
mcp_bundle_id = "tpp"
"#;
    ...
    assert_eq!(config.mcp_bundle_id.as_deref(), Some("tpp"));
    Ok(())
}
...
async fn permission_profile_override_preserves_split_write_roots() -> std::io::Result<()> {
    let sprite_home = TempDir::new()?;
    let cwd = sprite_home.path().join("workspace");
    let outside_root = sprite_home.path().join("outside-write");
...
export_dir = "locks"
allow_sprite_version_mismatch = true
...
"#,
    ...
    assert!(config.config_lock_allow_sprite_version_mismatch);
...
async fn debug_config_lockfile_load_path_loads_lock_from_nested_table() -> std::io::Result<()> {
    let sprite_home = TempDir::new()?;
    let lock_path = sprite_home.path().join("session.config.lock.toml");
    std::fs::write(
        &lock_path,
        format!(
            r#"version = {}
sprite_version = "older-version"
...
-> std::io::Result<()> {
    let sprite_home = TempDir::new()?;
    let selected_config = sprite_home.path().join("default.config.toml");
    std::fs::write(
        &selected_config,
        r#"approvals_reviewer = "user"
"#,
...
#[tokio::test]
async fn multi_agent_config_from_feature_table() -> std::io::Result<()> {
    let sprite_home = TempDir::new()?;
    std::fs::write(
        sprite_home.path().join(CONFIG_TOML_FILE),
        r#"[features.multi_agent]
enabled = true
...
"#,
    ...
    assert!(config.features.enabled(Feature::MultiAgent));
    assert_eq!(config.multi_agent.max_concurrent_threads_per_session, 5);
    assert_eq!(config.multi_agent.min_wait_timeout_ms, 2500);
    assert_eq!(config.multi_agent.max_wait_timeout_ms, 120000);
    assert_eq!(config.multi_agent.default_wait_timeout_ms, 30000);
    assert_eq!(
        (
    ...
    assert!(!config.multi_agent.usage_hint_enabled);
    assert_eq!(
        config.multi_agent.usage_hint_text.as_deref(),
        Some("Custom delegation guidance.")
    );
    assert_eq!(
        config.multi_agent.root_agent_usage_hint_text.as_deref(),
        Some("Root guidance.")
    );
    assert_eq!(
        config.multi_agent.subagent_usage_hint_text.as_deref(),
        Some("Subagent guidance.")
    );
    assert_eq!(
        config.multi_agent.tool_namespace.as_deref(),
        Some("agents")
    );
    assert!(config.multi_agent.hide_spawn_agent_metadata);
    assert!(config.multi_agent.non_code_mode_only);

    Ok(())
}

#[tokio::test]
async fn multi_agent_default_session_thread_cap_counts_root() -> std::io::Result<()> {
    let sprite_home = TempDir::new()?;
    std::fs::write(
        sprite_home.path().join(CONFIG_TOML_FILE),
        r#"[features.multi_agent]
...
"#,
    ...
    assert_eq!(config.multi_agent.max_concurrent_threads_per_session, 4);
    assert_eq!(config.multi_agent.min_wait_timeout_ms, 10_000);
    assert_eq!(config.multi_agent.max_wait_timeout_ms, 3_600_000);
    assert_eq!(config.multi_agent.default_wait_timeout_ms, 30_000);
    assert_eq!(
        (
            config.agent_max_threads,
            config.effective_agent_max_threads(MultiAgentVersion::V2)
...
#[tokio::test]
async fn multi_agent_empty_usage_hint_overrides_clear_default_hints() -> std::io::Result<()> {
    let sprite_home = TempDir::new()?;
    std::fs::write(
        sprite_home.path().join(CONFIG_TOML_FILE),
        r#"[features.multi_agent]
...
"#,
    ...
    assert_eq!(config.multi_agent.root_agent_usage_hint_text, None);
    assert_eq!(config.multi_agent.subagent_usage_hint_text, None);

    Ok(())
}

#[tokio::test]
async fn multi_agent_feature_rejects_agents_max_threads() -> std::io::Result<()> {
    let sprite_home = TempDir::new()?;
    std::fs::write(
        sprite_home.path().join(CONFIG_TOML_FILE),
        r#"[features.multi_agent]
...
"#,
    ...
    let err = config
        ...
        .expect_err("agents.max_threads should conflict with multi_agent");

    assert_eq!(err.kind(), std::io::ErrorKind::InvalidInput);
    assert_eq!(
        err.to_string(),
        "agents.max_threads cannot be set when features.multi_agent is enabled"
    );
    assert_eq!(
        config.effective_agent_max_threads(MultiAgentVersion::V2),
        Some(3)
...
"#,
    ...
    config.validate_multi_agent_config()?;
    assert_eq!(
        config.effective_agent_max_threads(MultiAgentVersion::V2),
        Some(3)
    );
...
#[tokio::test]
async fn multi_agent_rejects_invalid_wait_timeouts() -> std::io::Result<()> {
    let sprite_home = TempDir::new()?;
    std::fs::write(
        sprite_home.path().join(CONFIG_TOML_FILE),
        r#"[features.multi_agent]
...
"#,
    ...
    assert_eq!(config.multi_agent.min_wait_timeout_ms, 0);
    assert_eq!(config.multi_agent.max_wait_timeout_ms, 0);
    assert_eq!(config.multi_agent.default_wait_timeout_ms, 0);
...
"#,
    ...
    assert_eq!(
        err.to_string(),
        "features.multi_agent.min_wait_timeout_ms must be at least 0"
    );
...
"#,
    ...
    assert_eq!(
        err.to_string(),
        "features.multi_agent.min_wait_timeout_ms must be at most 3600000"
    );
...
"#,
    ...
    assert_eq!(
        err.to_string(),
        "features.multi_agent.max_wait_timeout_ms must be at least 0"
    );
...
"#,
    ...
    assert_eq!(
        err.to_string(),
        "features.multi_agent.max_wait_timeout_ms must be at most 3600000"
    );
...
"#,
    ...
    assert_eq!(
        err.to_string(),
        "features.multi_agent.default_wait_timeout_ms must be at least 0"
    );
...
"#,
    ...
    assert_eq!(
        err.to_string(),
        "features.multi_agent.min_wait_timeout_ms must be at most features.multi_agent.max_wait_timeout_ms"
    );
...
"#,
    ...
    assert_eq!(
        err.to_string(),
        "features.multi_agent.default_wait_timeout_ms must be at least features.multi_agent.min_wait_timeout_ms"
    );
...
"#,
    ...
    assert_eq!(
        err.to_string(),
        "features.multi_agent.default_wait_timeout_ms must be at most features.multi_agent.max_wait_timeout_ms"
    );
...
#[tokio::test]
async fn multi_agent_rejects_invalid_tool_namespace() -> std::io::Result<()> {
    for (namespace, expected_message) in [
        (
            "bad namespace",
            "features.multi_agent.tool_namespace must match ^[a-zA-Z0-9_-]+$",
        ),
        (
            "functions",
            "features.multi_agent.tool_namespace uses a reserved namespace: functions",
        ),
    ] {
        let sprite_home = TempDir::new()?;
        std::fs::write(
...
"#
        ...
        let err = ConfigBuilder::without_managed_config_for_tests()
            ...
            .build()
            .await
            .expect_err("invalid multi_agent tool namespace should fail");
        assert_eq!(err.kind(), std::io::ErrorKind::InvalidInput);
...
#[tokio::test]
async fn multi_agent_session_thread_cap_one_disallows_subagents() -> std::io::Result<()> {
    let sprite_home = TempDir::new()?;
    std::fs::write(
        sprite_home.path().join(CONFIG_TOML_FILE),
        r#"[features.multi_agent]
...
"#,
    ...
    assert_eq!(config.multi_agent.max_concurrent_threads_per_session, 1);
    assert_eq!(
        (
            config.agent_max_threads,
            config.effective_agent_max_threads(MultiAgentVersion::V2)
