...
use utils_pty::ExecCommandSession;
...
use crate::protocol::ResizeParams;
use crate::protocol::ResizeResponse;
...
impl LocalProcess {
    ...
    async fn start_process(
        &self,
        params: ExecParams,
    ) -> Result<(ExecResponse, watch::Sender<u64>, ExecProcessEventLog), ExecServerError> {
        let process_id = params.process_id.clone();
        let (program, args) = params
            .argv
            .split_first()
            .ok_or_else(|| ExecServerError::Protocol("argv must not be empty".to_string()))?;
        ...
        let spawned_result = if params.tty {
            utils_pty::spawn_pty_process(
                program,
                args,
                params.cwd.as_path(),
                &env,
                &params.arg0,
                params.terminal_size.into(),
            )
            .await
        } else if params.pipe_stdin {
            utils_pty::spawn_pipe_process(program, args, params.cwd.as_path(), &env, &params.arg0)
                .await
        } else {
            utils_pty::spawn_pipe_process_no_stdin(
                program,
                args,
                params.cwd.as_path(),
                &env,
        ...
        let spawned = match spawned_result {
            ...
            Err(err) => {
                ...
                return Err(ExecServerError::Spawn(err.to_string()));
            }
        };
        ...
        loop {
            let (response, output_notify) = {
                ...
                let process = process_map.get(&params.process_id).ok_or_else(|| {
                    ExecServerError::Protocol(format!("unknown process id {}", params.process_id))
                })?;
    ...
    pub(crate) async fn exec_write(
        ...
        writer_tx
            ...
            .await
            .map_err(|_| {
                ExecServerError::Protocol("failed to write to process stdin".to_string())
            })?;
    ...
    pub(crate) async fn resize_process(
        &self,
        params: ResizeParams,
    ) -> Result<ResizeResponse, ExecServerError> {
        let resized = {
            let process_map = self.inner.processes.lock().await;
            let Some(process) = process_map.get(&params.process_id) else {
                return Err(ExecServerError::Protocol(format!(
...
impl ExecProcess for LocalExecProcess {
    ...
    async fn resize(&self, size: TerminalSize) -> Result<ResizeResponse, ExecServerError> {
        self.backend.resize(&self.process_id, size).await
    }
}
...
impl LocalProcess {
    ...
    async fn resize(
        ...
        self.resize_process(ResizeParams {
            process_id: process_id.clone(),
            size,
        })
        .await
...
mod tests {
    ...
    #[tokio::test]
    async fn resize_process_forwards_size_to_pty_session() {
        let backend = LocalProcess::default();
        let (size_tx, size_rx) = oneshot::channel();
        let process = spawn_test_pty_process(&backend, "proc-resize", size_tx).await;
    ...
    async fn spawn_test_process(backend: &LocalProcess, process_id: &str) -> TestProcess {
        spawn_test_process_with_session(backend, process_id, dummy_session(), false).await
    }

    async fn spawn_test_pty_process(
        backend: &LocalProcess,
        process_id: &str,
        size_tx: oneshot::Sender<utils_pty::TerminalSize>,
    ) -> TestProcess {
        let mut size_tx = Some(size_tx);
        let session = dummy_session_with_resizer(Box::new(move |size| {
            if let Some(size_tx) = size_tx.take() {
                size_tx.send(size).expect("send resize");
            }
            Ok(())
        }));
        spawn_test_process_with_session(backend, process_id, session, true).await
    }

    async fn spawn_test_process_with_session(
        backend: &LocalProcess,
        process_id: &str,
        session: ExecCommandSession,
        tty: bool,
    ...
    fn dummy_session() -> ExecCommandSession {
        dummy_session_with_resizer(Box::new(|_| Ok(())))
    }

    fn dummy_session_with_resizer(
        resizer: Box<dyn FnMut(utils_pty::TerminalSize) -> anyhow::Result<()> + Send>,
    ) -> ExecCommandSession {
        ...
        utils_pty::spawn_from_driver(ProcessDriver {
            writer_tx,
            stdout_rx,
