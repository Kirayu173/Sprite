...
use crate::landlock::SPRITE_LINUX_SANDBOX_ARG0;
...
pub struct SandboxTransformRequest<'a> {
    ...
    pub sprite_linux_sandbox_exe: Option<&'a Path>,
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::MissingLinuxSandboxExecutable => {
                write!(f, "missing sprite-linux-sandbox executable path")
            }
            #[cfg(target_os = "linux")]
    ...
    pub fn transform(
        ...
        let (argv, arg0_override) = match sandbox {
            ...
            SandboxType::LinuxSeccomp => {
                let exe = sprite_linux_sandbox_exe
...
fn linux_sandbox_arg0_override(exe: &Path) -> String {
    if exe.file_name().and_then(|name| name.to_str()) == Some(SPRITE_LINUX_SANDBOX_ARG0) {
        os_string_to_command_component(exe.as_os_str().to_owned())
    } else {
        SPRITE_LINUX_SANDBOX_ARG0.to_string()
    }
}
