...
mod tests {
    ...
    fn windows_powershell_full_path_is_safe() {
        ...
        if crate::command_safety::try_parse_powershell_ast_commands(powershell, "Get-Location")
            .is_none()
        {
            eprintln!(
                "PowerShell AST parser unavailable; skipping windows_powershell_full_path_is_safe"
            );
            return;
        }
    ...
    fn non_windows_safe_classification_does_not_spawn_repo_powershell_path() {
        ...
        let temp_dir = std::env::temp_dir().join(format!(
            "sprite-safe-command-pwsh-test-{}-{unique}",
            std::process::id()
        ));
